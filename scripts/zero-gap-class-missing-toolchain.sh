...
#   Host tools/libraries whose ABSENCE makes a gate exit 2 (never a pass). Each absent one is
#   an Operator-blocked item: the finding carries the exact install command and the gate(s) that
#   stay rc 2 because of it. Installing anything is an operator action; this class never installs,
#   never runs sudo, and only reads.
#
...
#
# FINDINGS
#   tool:<t>     medium host-capability   catalogued tool absent (install command + gate in text).
...
#
# PATH SEMANTICS. The probe uses the PATH this process received (the runner's env -i allow-list).
#   A tool visible only through an unusual PATH (this host: sqlite3 lives only in the Android SDK
#   platform-tools directory) counts as MISSING for the runner when that PATH is not passed.
...
#
# MEASURED (live tree, 2026-09-26, review fix round, PATH=/usr/local/bin:/usr/bin:/bin; three runs
#   byte-identical, about 0.5 s): 30 items, 9 FINDINGs — 3 medium tool: (bundle, jekyll, tesseract),
#   1 high derived:node (_tools/portfolio/self-validate.sh:171 exits 2), 5 low derived: (docs_chain,
#   glab, ifconfig, lumen, shellcheck: every probe site is an optional/fallback lookup). All 9
#   hand-read: the 3 tool rows and node are real Operator-blocked items on that PATH (node and glab
#   do exist under ~/.nvm and /snap/bin, outside that PATH — see PATH SEMANTICS); the 5 low items
#   are real catalogue lag with no rc-2 consequence, which is what LOW states.
#
# USAGE
...
prove_failure() {
    ...
    : >"$work/w/clean/docs/zero-gap/toolchains.tsv"; printf 'tool\tprobe\tgate_that_needs_it\tinstall_command\tsource\n' >"$work/w/clean/docs/zero-gap/toolchains.tsv"
    out=$(run --corpus "$work/w/clean" 2>&1); rc=$?
    ...
    want=$(run --corpus "$work/w/planted" --emit-population 2>&1 | sha256sum | cut -d' ' -f1)
    ...
    fresh; printf '..\tcommand -v ..\tgate x\tnone\tsrc\n' >>"$work/w/clean/docs/zero-gap/toolchains.tsv"
    out=$(run --corpus "$work/w/clean" 2>&1); rc=$?
...
awk 'BEGIN { RS = ORS = "\0" } index($0, "'"$FIXPFX"'") != 1' "$W/all.z" >"$W/files.z"
...
awk -F'\t' -v OFS='\037' '
    /^[[:space:]]*$/ || /^#/ { next }
    !hdr { hdr = 1; if ($0 != "tool\tprobe\tgate_that_needs_it\tinstall_command\tsource") print "BADHEADER", NR; next }
    { bad = (NF != 5); for (i = 1; i <= NF; i++) if ($i == "") bad = 1
      if (bad) print "BADROW", NR; else print "ROW", NR, $1, $2, $3, $4, $5 }' "$CAT" >"$W/cat.txt"

declare -A CAT_STATE=() CAT_LINE=() CAT_GATE=() CAT_INSTALL=() CAT_PROBE=() CAT_KIND=() CAT_TARGET=() SEEN=()
POP=()
while IFS=$'\037' read -r kind ln tool probe gate inst src; do
    case "$kind" in
    BADHEADER) cni "$CAT_REL:$ln" "the catalogue header is not the expected five-column header"; continue ;;
    BADROW) cni "$CAT_REL:$ln" "a catalogue row does not have five non-empty tab-separated columns"; continue ;;
    esac
...
for item in $POPTXT; do
    ...
    tool:*)
        ...
        if [ "$state" = absent ]; then
            ...
            finding medium host-capability "$item" "Operator-blocked: $t is absent (probe: ${CAT_PROBE[$t]}); gate that exits 2 (never a pass) until installed: $gate.$users Install (operator action, never run by the sweep): ${CAT_INSTALL[$t]}" "$(zg_pct_encode "$ref")"
        else
            hit=$(docs_hit "$t")
            if [ -n "$hit" ]; then
    ...
    derived:*)
        ...
        if ! have_path "$t"; then
            ...
            if [ -n "${DER_CONS[$t]:-}" ]; then
                finding high governance-drift "$item" "catalogue missing derived tool: $t is absent on this host and has no row in $CAT_REL, and the probe at ${DER_CONS[$t]} is followed by exit 2 / return 2 / an undet call, so that gate exits 2 (never a pass) here ($n precondition(s) in total); add a row with its install command" "$(zg_pct_encode "${DER_CONS[$t]}")"
            else
                finding low governance-drift "$item" "catalogue missing derived tool: $t is absent on this host and has no row in $CAT_REL; none of its $n probe(s) (first $first) is followed by exit 2 / return 2 / an undet call, so no gate is shown to exit 2 on it; add a row or record why it is optional" "$(zg_pct_encode "$first")"
            fi
        fi ;;
    esac
done
