...
**V-G10** every `open` item has a `plan_due` date; **V-G2** `classified` ⇒ reason ∈ 4 + owner + `recheck` not elapsed at `--as-of`; **V-G3** `closed` ⇒ RED+GREEN evidence for the same check id + independent verdict (`item_verdicts`)
