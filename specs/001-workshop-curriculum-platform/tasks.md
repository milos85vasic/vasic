...
- [ ] T037 [US1] Produce and publish the accuracy report for Chapter 1 with the measured figure and its confidence interval — **[PATH NOT BUILT — `workshop/chapters/01/transcript/` does not exist, so neither does `accuracy-report.md`, and no accuracy figure has ever been produced anywhere: `find workshop -name 'accuracy*.json'` returns zero files. Note the destination question this raises and does not settle: T030's transcript landed under `workshop/curriculum/chapter-01/`, not `workshop/chapters/01/transcript/`, so whether the report belongs beside the transcript or beside the recording is an open decision — **CANNOT DETERMINE** from the tree, and it must be settled before this task is dispatched. Publish precondition B2 (T087) reads `accuracy.json`, which T112's wrapper writes, so the two paths must agree]**
...
> question this block does not answer, and T078's measured `SC-010 NOT met (3 fabrications)` is the
> evidence that it is a live question. T069's `extractive` adapter remains the zero-generative route
> and is unaffected.
...
| `workshop/scripts/verify-accuracy.sh` | T112 | BUILT — and NEVER RUN against the real chapter (no `accuracy*.json` exists anywhere) |
