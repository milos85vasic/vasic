...
**Eleven** genuinely unresolved items across the seven specifications, deduplicated, plus two
raised by the merge itself (fifteen total rows below, four — OQ-3, OQ-5, OQ-7 and OQ-9 — since
**RESOLVED**, see Clarifications "OQ-3 resolved during this pass", "OQ-5 resolved during this
pass", "OQ-7 resolved during this pass" and "OQ-9 resolved during this pass"; kept in this table,
not renumbered, for traceability). **None of the eleven open rows is a clarification marker left
in the text**; each is an owned decision awaiting an operator, and each states why it could not be
resolved under the safest-option rule.
