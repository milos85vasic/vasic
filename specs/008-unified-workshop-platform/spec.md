...
| **M6** | Cutover: pre-rollout sessions invalidated, anonymous per-browser progress explicitly handled (FR-321), umbrella copies decommissioned | The only irreversible step, and the only one that may not be started before M4 passes. **Every step in this gate is destructive and is therefore subject to FR-338**: each requires a preceding, existence-asserted hardlinked backup before it runs. |
...
**33** — eight merges where two specs described one journey, and no deletions. **The reduction is
seven, not eight, and the arithmetic closes for a stated reason**: one source story (`004/US6`)
contributes to **two** of the merged stories — US26 and US27 — so it is folded twice and removed
once, which is why forty sources and eight merges leave thirty-three rather than thirty-two. The
individual story entries state this; the summary sentence previously did not. Every story carries a
...
11. **Given** an unknown username and a known username with a wrong password, **When** each login fails, **Then** the response content and shape are identical. *(Content and shape only — response **timing** is explicitly out of scope per FR-312, and this scenario must not be read as closing the timing side channel.)*
...
**Two further requirements — FR-338 and FR-339 — were added by the 2026-09-15 remediation pass and
have no source-specification origin**, bringing the sequence to **339**. They are marked
`*(new, derived from remediation of the 2026-09-15 /speckit-analyze report)*` in place of a
back-reference, and both serve **both** architectural states. See Clarifications, "Remediation pass
— 2026-09-15".
...
(FR-286 – FR-337) describe the target architecture. FR-338 and FR-339 sit after both and serve
both.** A requirement that serves both states says so in its own text.
...
- **FR-012** *(was: 001/FR-039)*: The system MUST provide a documented redaction step capable of suppressing identified passages, and MUST require it to have been run before any export or publication of chapter content. Redaction and purge are destructive operations and are therefore subject to **FR-338**: a hardlinked backup MUST precede the step and its existence MUST be asserted before the step proceeds.
- **FR-013** *(was: 001/FR-050)*: Automated-check evidence that could contain transcript excerpts, speaker attribution, or other content derived from the private `workshop` submodule MUST be written to a location inside that submodule's own boundary, never into the public monorepo's shared evidence path. Only evidence provably free of such content MAY use the shared convention. *(**Baseline framing.** "That submodule's own boundary" names a public/private **submodule** relationship — `workshop` inside the public `vasic` umbrella — which Part II's target architecture dissolves: once `workshop` is a standalone repository it is no longer a submodule of anything. In the target architecture the rule is unchanged in substance and re-anchored in form: private-derivable evidence is written **inside the standalone module's own repository boundary**, and that repository's visibility is what carries the protection. For `ai_interviewing` that boundary is settled — the repository stays **PRIVATE** with credentialed access, per OQ-7's resolution (2026-09-15), so the boundary holds by construction. **For `workshop` it is not settled**: whether the standalone repository is PUBLIC or PRIVATE is **OQ-14**, itself additionally blocked on OQ-1, and this document does not assume an answer. Until OQ-14 is decided, evidence that could carry private-derivable `workshop` content MUST be treated as requiring a private destination, because writing it into a repository whose visibility is undecided is the one outcome that cannot be undone.)*
...
- **FR-081** *(was: 002/FR-008b)*: An already-published area whose evidencing passages are reduced to zero by a later redaction MUST be automatically unpublished — or held for an explicit republication decision — and MUST NOT continue to display, export or resolve as though it still carried live evidence. Automatic unpublication is a destructive operation and is therefore subject to **FR-338**: a hardlinked backup MUST precede it and its existence MUST be asserted first.
...
- **FR-264** *(was: 001/FR-040, 002/FR-055 — MERGED)*: Checks MUST write their evidence to a versioned location within the repository, retained alongside the commit that produced it, rather than to transient logs. *(Which repository is governed by FR-013: anything that could carry private content goes inside the private submodule's own boundary. **In the target architecture** the "private submodule" is no longer a submodule — it is the standalone module's own repository, and the destination is that repository's boundary: settled and PRIVATE for `ai_interviewing` per OQ-7, **undecided for `workshop` per OQ-14**, which per FR-013's Part II note means a private destination until OQ-14 resolves.)*
...
- **FR-272** *(was: 001/FR-035)*: All governance carrier documents MUST remain synchronised, and the continuation document MUST be updated alongside non-trivial changes. *(This governs carriers that already exist. For a repository this work **creates or extracts**, the carriers MUST exist and be manifest-declared before its first push — see FR-339.)*
...
depends on them. Where a Part II requirement supersedes a Part I one, both say so. FR-338 and
FR-339, which follow this block, are **not** Part II requirements — they bind both states.*
...
- **FR-288** *(was: 007/FR-003, 004/FR-024 — MERGED)*: Both modules MUST be clonable and runnable on a machine with **no access to the `vasic` umbrella repository**, obtaining every dependency through their own documented bootstrap. *(Merged: 004/FR-024 stated this as a P3 baseline property of each learning module; 007/FR-003 states it as the primary objective of the migration. One requirement, two framings, and the target framing governs.)* **For `ai_interviewing`, RESOLVED by OQ-7 (2026-09-15, operator decision): satisfied by a PRIVATE repository with invited/credentialed collaborator access, not by public visibility.** "No access to the umbrella" is satisfied by direct credentials to this one repository; it does not require anyone-can-clone. For `workshop`, this remains open — see OQ-14, which is additionally blocked on the still-unresolved OQ-1.
...
- **FR-302** *(was: 007/FR-015)*: Users MUST be able to switch accounts — sign out and sign in as a different user — completing the switch **within the 3-second bound SC-150 states**, with no manual step beyond the sign-out and sign-in themselves. *("Seamlessly" was replaced by SC-150's actual bound during the 2026-09-15 remediation pass; the adjective named no threshold and nothing measured it.)*
...
- **FR-304** *(was: 007/FR-017)*: Session management MUST be secure: session cookies MUST be `HttpOnly`, `Secure` and carry an explicit `SameSite` attribute; state-changing requests MUST carry CSRF protection; and every response from the authenticated surface MUST carry, at minimum, **`Strict-Transport-Security`**, **`X-Content-Type-Options: nosniff`**, a **frame-ancestors control** (`X-Frame-Options`, or an equivalent `Content-Security-Policy` `frame-ancestors` directive — either satisfies this, and a document carrying both MUST NOT let them disagree), and a **`Content-Security-Policy`** covering at least the authentication surface. *(Enumerated during the 2026-09-15 remediation pass. "Secure headers" named no header set and nothing measured it, so the requirement could be satisfied by any one header or by none; the enumeration is the requirement's explicit content and is measured by **SC-168** against a live authenticated response. The list is a floor, not a ceiling — adding a header is compliance, omitting one on this list is not.)*
...
- **FR-312** *(was: 007/FR-049)*: Authentication failure messages MUST be generic and MUST NOT disclose whether a given username exists. The response content and shape MUST be identical for an unknown username and a known username with a wrong password. **Response TIMING is explicitly OUT OF SCOPE for this specification.** This requirement governs content and shape only; it makes no claim about, and no measurement of, the timing differential between the two paths — the side channel this exact guard is known to leak through, since an unknown username typically short-circuits before any password hash is computed while a known one does not. That gap is recorded here as a **stated, deliberate boundary rather than an implied-but-unaudited one**: a reader must not infer from FR-312 that username enumeration is closed. Naming a concrete millisecond bound is not attempted, because no measurement of this platform's login path exists and inventing a threshold would be the guess §11.4.6 forbids; setting one is available as a later, evidenced decision. *(Boundary stated during the 2026-09-15 remediation pass.)*
...
- **FR-315** *(was: 007/FR-055)*: Any session or cookie issued before secure session handling (FR-304) was enforced MUST be invalidated at the authentication-rollout cutover. Mass session invalidation is a destructive operation and is therefore subject to **FR-338**: the session store MUST carry a preceding, existence-asserted hardlinked backup before invalidation runs.
...
- **FR-317** *(was: 007/FR-018)*: On the workshop module, every learner action across areas, lessons, tests and exercises MUST be **recorded** when it occurs and **restored** when the user next authenticates — the observable outcome SC-151 measures at 100%. *("Fully implemented" was restated as its observable outcome during the 2026-09-15 remediation pass: it named no object, so nothing could be said to have satisfied it. SC-151 already supplied the real criterion; this requirement now states it rather than pointing at an unbounded adjective.)*
- **FR-318** *(was: 007/FR-019)*: On the `ai_interviewing` module, every learner action across areas, lessons, tests and exercises MUST be **recorded** when it occurs and **restored** when the user next authenticates — the same observable outcome, measured on the second module by the same SC-151. *(Restated during the 2026-09-15 remediation pass for the same reason as FR-317.)*
...
- **FR-321** *(new, derived from the merge — see Clarifications C3)*: At the authentication cutover, any anonymous per-browser progress record created under the baseline architecture MUST be handled by a **stated, documented outcome** — migrated on an explicit learner action, or discarded with the learner told. It MUST NOT be silently merged into a user account (which would attribute one person's activity to another on a shared browser) and MUST NOT be silently discarded (which would destroy a record FR-118 promised to preserve). Where the stated outcome is discard, that discard is a destructive operation and is subject to **FR-338**: a hardlinked backup of the anonymous progress records MUST precede it and MUST be asserted to exist first.

#### Reusable-submodule extraction

- **FR-322** *(was: 007/FR-022 — SCOPE REDUCED TO ITS DELTA over FR-285)*: **FR-285 is the normative statement of where a reusable component lives** — its own repository under an owned organisation, on both GitHub and GitLab, consumed as a submodule mounted at the project root, nesting forbidden — and it already binds both architectural states. FR-322 adds only what FR-285 does not state: **the identification rule that decides what gets extracted, and the completeness obligation that the identified set is fully extracted.** A unit of code is **reusable, and therefore in scope for extraction, if either** (a) it is used by more than one module, **or** (b) it contains no curriculum-specific type and depends on none — the property FR-281 already requires of everything this work produces and FR-283 already requires to be independently testable. The identified set MUST be enumerated, each member classified against (a) or (b) with the evidence for that classification recorded, and **every** member of the identified set MUST be extracted; a member identified and not extracted MUST be reported with its reason rather than silently omitted. Extraction itself then proceeds under FR-285, FR-327 (content-boundary review before the public repository exists) and FR-339 (carriers and manifest before first push). *(Scope reduced during the 2026-09-15 remediation pass: as written in 007 this independently restated FR-285's ground while stating no criterion for "all reusable code", which made it unmeasurable. See Clarifications, "Remediation pass — 2026-09-15", items U3 and A2.)*
- **FR-323** *(was: 007/FR-025)*: The GitHub and GitLab command-line interfaces MUST be used for repository creation.
- **FR-324** *(was: 007/FR-026)*: An extracted submodule MUST satisfy FR-322's criterion (b) **in the extracted artifact itself, not merely in intent**: it MUST contain no curriculum-specific type, MUST depend on none, and MUST build and pass its own test suite with neither module present — which is FR-281 and FR-283 asserted against the extracted repository rather than against the pre-extraction source. Genericity is therefore measured, not declared. *(Restated during the 2026-09-15 remediation pass: "fully generic and reusable" named no criterion, and FR-322's rule is the criterion it was reaching for.)*
- **FR-325** *(was: 007/FR-027 — SCOPE REDUCED TO ITS DELTA over FR-285)*: **FR-285 governs the consumption form** — a git submodule mounted at the project root, nesting forbidden — in both states. FR-325's Part-II-specific delta is the **mount point and the resolution path**: each standalone module MUST mount every extracted submodule at **its own repository root** and resolve it through **its own `.gitmodules`** (FR-289), never through the umbrella's `submodules/` directory or the umbrella's `helix-deps.yaml`. A consumption path that still resolves through the umbrella is not an external dependency; it is the coupling FR-288 exists to remove. *(Scope reduced during the 2026-09-15 remediation pass — see Clarifications, item A2.)*
...
- **FR-328** *(was: 007/FR-052)*: A module MUST NOT be decommissioned from its umbrella-hosted location until its standalone clone independently passes install **and** the full test suite; no module may be left with no fully-working copy. Decommissioning is a destructive operation and is additionally subject to **FR-338**: a hardlinked backup of the umbrella-hosted copy MUST precede it and MUST be asserted to exist before it proceeds. *(This is migration gate M4.)*
...
- **FR-330** *(was: 007/FR-029 — RESTATED AS AN UMBRELLA, NO INDEPENDENT MUST)*: **This requirement carries no independent obligation of its own.** "All documentation MUST be extended and updated" named no document set, so nothing could establish that it had been satisfied. The documentation obligation for both standalone modules is **exactly and only** the union of FR-331 (user guides, manuals, graphs, diagrams and schemes), FR-332 (template files and SQL definitions), FR-333 (a complete README with clone, install and run instructions), FR-334 (architecture diagrams showing module boundaries and data flows) and FR-336 (the user-facing set: quick-start tutorial, user guide, operator manual, FAQ). Satisfying all five satisfies FR-330; there is no sixth, unnamed document this requirement additionally demands. *(Restated as an umbrella reference during the 2026-09-15 remediation pass rather than narrowed to an invented document set — the union of the five is what the source requirement was reaching for, and inventing a further list would add an obligation no source specification states.)*
- **FR-331** *(was: 007/FR-030)*: User guides, manuals, graphs, diagrams and schemes MUST be created or updated **for each standalone module, and the set MUST be enumerated per module** — each artefact named, with its state recorded as created, updated, or unchanged-with-reason. The completeness criterion is the enumeration: an artefact kind listed here with no named artefact and no recorded reason is a finding. *(The completeness criterion was added during the 2026-09-15 remediation pass; the source requirement listed artefact kinds but stated no way to tell when the list had been discharged.)* *(The baseline's own documentation set — quick-start tutorial, user guide, operator manual, FAQ — is FR-336 and is not replaced by this.)*
- **FR-332** *(was: 007/FR-031)*: Every template file and every SQL definition **shipped by either standalone module** MUST be documented, and the named set MUST be derived by enumerating those files in each module's tree rather than from a hand-maintained list — a hand-maintained list is a list that drifts, and a template added later would silently escape it. A file enumerated and undocumented is a finding; the documented count and the enumerated count MUST be published together. *(The set was bound to a derivation during the 2026-09-15 remediation pass; the source requirement named no template set at all.)*
...
*The evidence discipline itself — automated coverage of every requirement (FR-259), data-driven
paired demonstrations (FR-260), three-valued outcomes with could-not-determine never recorded as a
pass (FR-261), non-empty-set establishment (FR-262), corpus-did-not-move evidence (FR-263),
versioned evidence locations (FR-264), a machine-produced artefact behind every capability claim
(FR-265), SOURCE-versus-SERVED declaration (FR-266), and retention without silent deletion
(FR-270) — **already binds both architectural states** and is **not** restated here. The three
requirements below state only what the target architecture adds on top of it.*

- **FR-335** *(was: 007/FR-034 — REDUCED TO ITS DELTA over FR-259 – FR-266)*: **The delta is the test-type enumeration.** FR-259 already requires every requirement to be covered by at least one automated check, but states nothing about the *kinds* of check. For the target architecture that enumeration is mandatory and closed: **unit, integration, end-to-end, contract, performance, security and accessibility** — every one of the seven MUST be present, and a type with zero tests MUST be reported by name rather than silently absent (which is FR-262's non-empty-set rule applied per test type). Everything else about how those checks behave, prove themselves and record their evidence is FR-259 – FR-266 and is not restated. *(Reduced during the 2026-09-15 remediation pass — see Clarifications, item A1.)*
- **FR-336** *(was: 007/FR-035, 007/FR-036 — MERGED; and 001/FR-030 for the baseline documentation set; REDUCED TO ITS DELTA)*: **The delta is the user-facing documentation set**: the system MUST ship a quick-start tutorial, a user guide, an operator manual and an FAQ. The evidence half of the source requirement — that every test produce machine-readable, deterministic evidence verifiable without human interpretation — **is FR-265, FR-266 and SC-139 and is not restated here**; it binds both states already and gains nothing from a second statement that could later drift from the first. *(Reduced during the 2026-09-15 remediation pass — see Clarifications, item A1.)*
- **FR-337** *(was: 007/FR-037, 007/FR-038 — MERGED; REDUCED TO ITS DELTA)*: **The delta is per-requirement traceability of the archived artefact**: every piece of test evidence MUST be traceable to the **specific requirement it satisfies**, so a requirement's coverage can be audited from the evidence rather than from a claim. The zero-tolerance-for-unearned-claims half is FR-265 and FR-269 and the retention half is FR-270; neither is restated. *(Reduced during the 2026-09-15 remediation pass — see Clarifications, item A1.)*

#### Remediation-pass requirements — serve BOTH states

*These two requirements were added by the 2026-09-15 remediation pass and have no source-spec
origin. They close two gaps the pass found: an inherited constitutional precondition that no
requirement imposed, and a governance obligation that nothing bound the repositories this work
creates. Both bind the baseline and the target. See Clarifications, "Remediation pass — 2026-09-15".*

- **FR-338** *(new, derived from remediation of the 2026-09-15 /speckit-analyze report)*: **No naked writes.** Every destructive operation this specification mandates MUST be preceded by a hardlinked backup taken under the constitution's §9 discipline, and the backup's existence MUST be asserted — not assumed — before the destructive step is allowed to proceed. If the backup cannot be taken, or its existence cannot be confirmed, the destructive step MUST refuse rather than continue. The operations this binds are, at minimum: redaction or purge of chapter material (FR-012, and the propagation FR-148/FR-149 require); automatic unpublication of an area whose evidencing passages are reduced to zero (FR-081); invalidation of pre-rollout sessions and cookies at the authentication cutover (FR-315); disposition of anonymous per-browser progress records at that same cutover (FR-321); and decommissioning of a module's umbrella-hosted copy (FR-328, migration gate M6). The list is a floor, not a closed set: any operation that removes, overwrites or renders unreachable material a prior state held is a destructive operation for the purposes of this requirement. *(The constitution states this as an unconditional precondition — "No naked writes — every destructive operation requires a hardlinked backup first (§9)" — and a specification may extend an inherited rule but never weaken it. Before this requirement, five mandated destructive operations carried no backup precondition anywhere in this document.)*
- **FR-339** *(new, derived from remediation of the 2026-09-15 /speckit-analyze report)*: Every repository this work **creates or extracts** — the standalone `workshop` repository (FR-286), the standalone `ai_interviewing` repository (FR-287), and every public reusable repository produced by the extraction FR-285/FR-322/SC-152 require — MUST, **before its first push to any remote**, (a) carry all four governance carrier documents (`CLAUDE.md`, `AGENTS.md`, `QWEN.md`, `GEMINI.md`), each opening with the `## INHERITED FROM` pointer heading that the inheritance contract recognises, and all four in lockstep with one another; and (b) be declared in a `helix-deps.yaml`-equivalent dependency manifest whose recorded ref equals the live gitlink of the consumer that mounts it. A repository that exists on a remote without its carriers is a repository that has already been cloned by someone who was never told the rules. *(This extends FR-272, which requires the carriers to remain synchronised but says nothing about repositories that do not yet exist. The obligation is stated at first-push time deliberately: after the push, the unsupervised clone has already happened and cannot be recalled.)*
...
added by this merge itself (SC-160 – SC-162). **A further three — SC-166, SC-167 and SC-168 — were
added by the 2026-09-15 remediation pass and have no source-specification origin, bringing the
sequence to 168**; they are marked
`*(new, derived from remediation of the 2026-09-15 /speckit-analyze report)*` in place of a
back-reference. Every
...
- **SC-004** *(was: 001/SC-004)*: A new user goes from a fresh clone to a running curriculum in under 15 minutes using only the quick-start tutorial. **Measured by** a reviewer's recorded observation sheet — one timed run per observation, the observer's prior familiarity with the platform recorded alongside the time, and at least 3 observations by distinct people so a single unusually-fluent run is not the whole evidence. **This criterion is human-judgement/observation-based and is deliberately not automatable**: an automated harness measures a script's runtime, not a person's, and would report a pass on a tutorial no human can follow. No automated gate should be built for it. *(Tagged during the 2026-09-15 remediation pass, matching SC-044's convention.)*
...
- **SC-102** *(was: 001/SC-011)*: A maintainer adds a new chapter using only the documented procedure, with zero code or configuration changes, in under **30 minutes** of hands-on time. **Measured by** a maintainer's recorded observation sheet stating the elapsed hands-on time and the diff, with the zero-code-or-configuration-change half asserted mechanically against that diff. **The timing half is human-observation-based and deliberately not automatable**; the diff half is not, and the two MUST be reported separately so an automated diff pass is never read as evidence about the 30 minutes. *(Tagged during the 2026-09-15 remediation pass, matching SC-044's convention.)*
...
- **SC-113** *(was: 006/SC-007)*: A tester unfamiliar with the platform completes the QA document **without asking a question**, and every step's expected result was stated before they performed it. **Measured by** the tester's own recorded run sheet — every question they needed to ask recorded verbatim, with zero questions the passing condition — the tester's unfamiliarity recorded as a precondition rather than assumed. **This criterion is human-judgement/observation-based and is deliberately not automatable**: whether a document is followable without asking is a property of a reader, and no gate can stand in for one. No automated gate should be built for it. *(Tagged during the 2026-09-15 remediation pass, matching SC-044's convention.)*
...
- **SC-115** *(was: 006/SC-015)*: A client-facing walkthrough of the platform completes with **0** steps that cannot be performed as described. **Measured by** the walkthrough presenter's recorded step-by-step sheet, each step marked performed-as-described or not, with any not-as-described step named rather than counted. **This criterion is human-observation-based and is deliberately not automatable**: a walkthrough is a performance in front of a client, and its failure mode — a step that technically succeeds but cannot be narrated as described — is invisible to a gate. No automated gate should be built for it. *(Tagged during the 2026-09-15 remediation pass, matching SC-044's convention.)*

### Presentation and accessibility

- **SC-116** *(was: 004/SC-010, 005/SC-012 — MERGED)*: The served interface presents at least **6** distinct hue families across its principal surfaces, up from **2**, and **100%** of hues present carry a stated classification job. **The two halves are one condition, not two, and the floor is conditioned on the second: a hue counts toward the ≥ 6 only if it satisfies FR-225's per-hue-job requirement.** Six hues each carrying a stated job is compliant; six hues of which any lacks a stated job is **not** compliant and does not become compliant by adding a seventh. **Measured by** enumerating the hue families on the principal surfaces (the population being FR-226's — the surfaces a visitor actually uses), pairing each with its stated job, and taking the count **over the paired set only**. Paired mutation: add a seventh hue with no stated job and assert the counted total does **not** rise — a count that rises is the gaming this condition exists to prevent. *(Conditioning stated during the 2026-09-15 remediation pass: FR-225 was the counterweight to a one-sided floor, and nothing bound the two together, so the floor could be met by adding meaningless hues.)*
...
- **SC-146** *(was: 007/SC-001, 004/SC-012 — MERGED)*: The workshop module clones and runs on a fresh machine with no umbrella access, reaching a self-reported ready state, in **under 10 minutes** with a single command. *(What "ready" means when the shared user store of FR-295 is not yet provisioned is **not stated by any artifact** and is recorded as **OQ-16**; this criterion is not measurable against a fresh clone until that is decided.)*
- **SC-147** *(was: 007/SC-002, 004/SC-012 — MERGED)*: The `ai_interviewing` module clones and runs on a fresh machine with no umbrella access, reaching a self-reported ready state, in **under 10 minutes** with a single command. *(Same unresolved definition of "ready" as SC-146 — see **OQ-16**.)*
...
- **SC-152** *(was: 007/SC-007)*: **The identified set is fully extracted, and the count is a consequence rather than the target.** Measured in three parts: (i) every unit of code satisfying FR-322's identification rule — used by more than one module, **or** containing and depending on no curriculum-specific type — is enumerated, with each member's classification and its supporting evidence recorded; (ii) **100%** of that enumerated set is extracted into its own public repository under the owned organisation on **both** GitHub and GitLab, with members identified but not extracted reported by name and reason and numbering **0** unreported; and (iii) each extracted repository independently satisfies FR-324 (builds and passes its own suite with neither module present) and FR-339 (four governance carriers in lockstep and a manifest declaration, both before first push — see SC-167). **At least 5** remains as the floor the source criterion recorded, but a run satisfying the floor while failing (i), (ii) or (iii) does **not** satisfy this criterion: five arbitrary repositories are not an extraction. Paired mutation: leave one enumerated member unextracted and unreported — the check must go red. *(Scope stated during the 2026-09-15 remediation pass; the bare count could previously be satisfied by extracting five things and identifying nothing. See Clarifications, items U3 and D2.)*
...
- **One thing the decision-record does not state, and this merge does not invent an answer to
  ...
  to task T184. **It has since been promoted to an owned row: OQ-17** (raised by the 2026-09-15
  remediation pass), after being flagged three times — here, in FR-036, and in the tasks file — with
  no owner. Promotion changes nothing about the answer: it remains an operator scheduling decision
  and no artifact names a release or a date.
...
### OQ-7 resolved during this pass

Different in kind from OQ-5 and OQ-9: not a case of no precedent existing, but a real disclosure
decision about real, currently-private content — `ai_interviewing` holds employer information and
GitHub-project analysis data (FR-299), and it is confirmed **private** today
(`gh api repos/milos85vasic/ai_interviewing --jq '.visibility'` → `private`). Neither reading is a
safe technical default: going public exposes real private content and — per this repository's own
standing governance — cannot later be undone; staying private risks under-serving FR-288's
cloneability goal if read too literally.

**Investigated before asking, to narrow the actual decision rather than widen it**: FR-288's own
text requires only that both modules be clonable "with no access to the `vasic` umbrella
repository" — it does not, read literally, require public visibility. A private repository with
invited or credentialed collaborator access already satisfies "no umbrella access," because the
credential is to the standalone repository itself, not to the umbrella. This meant the real choice
was not "PUBLIC or a broken requirement," but "PUBLIC or PRIVATE-with-credentials," which is a
materially smaller and more honest question to put to the operator.

**Put to the operator with no recommended default** — unlike OQ-5 and OQ-9, this decision carries
no clearly safer engineering option, only a real trade-off between disclosure risk and access
friction. **The operator chose PRIVATE, with invited/credentialed collaborator access.** FR-288 is
updated to record this as the specific satisfying interpretation for `ai_interviewing`; the
"going public cannot be undone, staying private can be reversed later" asymmetry was named as a
supporting consideration, never as a stand-in for the operator's own decision.

**Distinct question, deliberately not touched here**: OQ-14 asks the same visibility question of
the **`workshop`** repository, and it is NOT resolved by this entry — `workshop` holds different
content (a recording of a private teaching session with an identifiable third party) and is
additionally blocked on OQ-1, the still-open consent question. Resolving OQ-7 does not imply, and
must not be read as implying, any answer to OQ-14.
...
### Remediation pass — 2026-09-15

A structured `/speckit-analyze` run over this document returned a findings report; this entry
records **every change that pass made to this specification, and why**, in the same form as the four
OQ-resolution entries above. Nothing else in the feature directory was touched by this pass —
`plan.md` and `tasks.md` were remediated separately and concurrently by other work.

**The standing rule applied throughout is the one this document already uses**: where a choice was
mechanical, it was made; where it was genuinely an operator decision, **no answer was invented** and
the question was promoted to a numbered Open Question instead. Two findings took that second route
(F1 → OQ-16, F17 → OQ-17), and one was closed by stating a deliberate scope boundary rather than by
inventing a threshold (C1). Every count this pass wrote was recomputed against the live file after
all other edits landed, not carried from the report.

**New requirements, criteria and questions minted by this pass.** None carries a `*(was: NNN/FR-YYY)*`
back-reference, because none has a source-specification origin; each is marked
`*(new, derived from remediation of the 2026-09-15 /speckit-analyze report)*`, matching how
SC-160 – SC-162 are marked "new, derived from the merge".

| New item | Finding | What it closes |
|---|---|---|
| **FR-338** | D1 (critical) | The constitution states an unconditional precondition — *"No naked writes — every destructive operation requires a hardlinked backup first (§9)"* — and this document mandated **five** destructive operations (FR-012 redaction/purge, FR-081 auto-unpublish, FR-315 session invalidation, FR-321 anonymous-progress disposition, FR-328/M6 decommissioning) while **no requirement imposed the backup precondition on any of them**. A specification may extend an inherited rule and may never weaken it, and silence is a weakening here. FR-338 additionally requires the backup's **existence to be asserted**, not assumed, before the destructive step proceeds. |
| **SC-166** | D1 | Makes FR-338 measurable: 100% of destructive operations carry a preceding backup step, and its confirming assertion, in their own execution trace. Deliberately specified against the trace rather than the code, because a code path that *mentions* a backup is the exact false-green the constitution's own "an assertion that greps a file for a string is not a test" warns about. |
| **FR-339** | D2 (critical) | Part II creates two standalone repositories (FR-286, FR-287) and at least five extracted public reusables (SC-152), and **nothing required any of them to carry the four governance carriers or to be manifest-declared**. FR-272 governs carriers that already exist and says nothing about repositories that do not yet exist. The obligation is stated at **first-push** time on purpose: after the push, an unsupervised clone has already happened and cannot be recalled. |
| **SC-167** | D2 | Makes FR-339 measurable — carriers present, `## INHERITED FROM` pointer heading, four-way lockstep, manifest ref equal to the live gitlink, all before first push. |
| **SC-168** | U1 (high) | Makes FR-304's newly-enumerated header set measurable **on a live authenticated response**, never on a configuration file — a configuration assertion is precisely the SOURCE-versus-SERVED confusion FR-266 exists to prevent, and this family has recorded that same mistake five times. |
| **OQ-16** | F1 | **Not resolved here, by design.** SC-146/SC-147 promise a fresh clone reaching a self-reported "ready" state in under 10 minutes with one command; OQ-9's resolution made the user store a separately-hosted, network-consumed dependency that FR-294/FR-310 require both modules to **fail closed** against. A fresh clone with no store provisioned is therefore simultaneously correct and serving nothing, and **no artifact states whether that is "ready"**. The three candidate answers carry materially different costs, none is an engineering default, and inventing one would be the guess §11.4.6 forbids. SC-146 and SC-147 now carry a one-line pointer to this row. |
| **OQ-17** | F17 | **Not resolved here, by design.** The transitional dual-field wire state (`ordinal` and `ordinal_path` served side by side) is scoped in the live code to *"for one release"*, and **no artifact names that release or a date** — not the 003 decision-record, not FR-036, not the OQ-3 entry above, not the tasks file. Flagged three times with zero owner, which is why it is promoted to a numbered row rather than recorded as a fourth note. It is an operator **scheduling** decision, not a technical default: the removal date depends on when the consumers the zero-diff decision was chosen to protect are migrated, and no such plan exists. Cross-referenced from FR-036 and from the OQ-3 entry above. |

**In-place changes, each stating what it replaced.**

- **D1 cross-references.** FR-012, FR-081, FR-315, FR-321 and FR-328, and migration gate M6, each gained a short citing clause binding them to FR-338. None was rewritten: the five requirements were correct, they were merely missing a precondition.
- **D2 cross-references.** FR-272 gained a clause pointing at FR-339 for repositories that do not yet exist; SC-152 gained the same pointer as part of its restatement below.
- **F11 — FR-013 and FR-264, Part II scoping.** FR-013 requires private-derivable evidence to live *"inside that submodule's own boundary"* — a phrase that names a public/private **submodule** relationship the target architecture **dissolves**, since a standalone `workshop` is no longer a submodule of anything. Both now carry a Part II note matching the one FR-014 already carried: in the target state the destination is the standalone module's **own repository boundary**. For `ai_interviewing` that boundary is settled PRIVATE per OQ-7. **For `workshop` it is NOT settled** — OQ-14, itself blocked on OQ-1 — and this pass **did not assume an answer**; the note states that until OQ-14 resolves, a private destination is required, because writing into a repository of undecided visibility is the one outcome that cannot be undone.
- **U7 — FR-036 grammar.** *"is the ordering key MUST used wherever"* → *"MUST be used as the ordering key wherever"*. A pure grammar fix; every other word of that carefully-worded OQ-3-derived requirement is unchanged. (The same requirement separately gained the OQ-17 cross-reference recorded above.)
- **U1 — FR-304.** *"secure headers"* named no header set and nothing measured it, so the requirement was satisfiable by any one header or by none. It now enumerates `Strict-Transport-Security`, `X-Content-Type-Options: nosniff`, a frame-ancestors control (`X-Frame-Options` **or** an equivalent CSP `frame-ancestors` directive — either satisfies it), and a `Content-Security-Policy` covering at least the auth surface, plus explicit `HttpOnly`/`Secure`/`SameSite` cookie attributes. Stated as a **floor, not a ceiling**.
- **U2 — FR-317, FR-318.** Both read *"Progress tracking MUST be fully implemented"* — an adjective with no object, which nothing could be said to have satisfied. Both now state the observable outcome directly (every learner action **recorded** when it occurs and **restored** at next authentication) and cite SC-151, which already supplied the real criterion at 100%.
- **U3 — FR-322, FR-324, SC-152.** *"All reusable code MUST be identified and extracted"* stated no identification criterion, and *"fully generic"* stated no completeness criterion. FR-281 already implied the workable rule and it is now stated explicitly in FR-322: a unit is reusable if it is **used by more than one module**, **or** it contains and depends on **no curriculum-specific type**. FR-324 now requires that property to hold **in the extracted artifact**, proven by building and testing it with neither module present, rather than declared. SC-152 no longer measures a bare count: the ≥ 5 floor is retained as the source criterion's own figure, but the criterion is enumerate-classify-extract-all, and **five arbitrary repositories are not an extraction**.
- **U4 — FR-330, FR-331, FR-332.** FR-330 (*"All documentation MUST be extended and updated"*) is restated as an **umbrella with no independent MUST of its own**, referencing FR-331/FR-332/FR-333/FR-334/FR-336 — the less invasive of the two options, and the honest one: narrowing it to an invented document set would have added an obligation no source specification states. FR-331 gained a per-module enumeration as its completeness criterion; FR-332's *"template files and SQL definitions"* is now bound to a **derivation over each module's tree** rather than a hand-maintained list, because a hand-maintained list is a list a later-added template silently escapes. FR-333 and FR-336 were already testable and are unchanged.
- **U5 — FR-302.** *"seamlessly"* replaced by SC-150's actual bound: the switch completes **within 3 seconds**, with no manual step beyond the sign-out and sign-in themselves.
- **A1 — FR-335, FR-336, FR-337.** All three substantially restated evidence discipline that **FR-259 – FR-266 already establishes for both architectural states**. Each is reduced to its genuine delta with an explicit cross-reference: FR-335 keeps **only the seven-test-type enumeration** (FR-259 requires coverage but says nothing about kinds); FR-336 keeps **only the user-facing documentation set**; FR-337 keeps **only per-requirement traceability of the archived artefact**. The surrounding block gained a lead-in naming the nine requirements that are not restated. A second statement of the same ground is a second thing that can drift from the first.
- **A2 — FR-322, FR-325.** FR-285 is kept as **the normative statement** of where a reusable component lives (own repository, GitHub **and** GitLab, root-mounted, nesting forbidden — binding both states already). FR-322 is reduced to the identification and completeness rule recorded under U3; FR-325 is reduced to its Part-II-specific delta — mounting at **each standalone module's own root** and resolving through **its own `.gitmodules`**, never through the umbrella's `submodules/` or `helix-deps.yaml`, because a path that still resolves through the umbrella is the coupling FR-288 exists to remove.
- **C1 — FR-312 and US28 acceptance scenario 11.** Both correctly require an unknown-username and a wrong-password response to be identical in content and shape, and both said nothing about **timing** — the standard side channel this exact guard leaks through, since an unknown username typically short-circuits before any password hash is computed. Option (b) was taken: **timing-channel resistance is declared explicitly OUT OF SCOPE**, so the gap is a stated, deliberate boundary rather than an implied-but-unaudited one. Option (a) was rejected because no measurement of this platform's login path exists and **naming a millisecond bound would have been exactly the invention §11.4.6 forbids**; setting one remains available as a later, evidenced decision.
- **C2 — SC-116.** SC-116's ≥ 6 hue floor is one-sided, and FR-225's per-hue-job requirement is the counterweight that prevents gaming it by adding meaningless hues — but **nothing bound the two together**. SC-116 now states that the count is taken over the **paired set only**: six hues each carrying a stated job is compliant; six hues of which any lacks one is not, and does not become compliant by adding a seventh. Its paired mutation asserts the count does **not** rise when a job-less hue is added. Task-level gating was deliberately not touched — that is a `tasks.md` concern handled separately.
- **C3 — OQ-15.** OQ-15's "why it cannot be resolved here" column gained a **stated consequence** so the operator sees the real cost of each branch before choosing: under the written-against-the-target reading, the QA and client documents (FR-206 – FR-217) cannot be finalized until the full migration completes, which means the operator's own repeatedly-stated **absolute-priority** milestone — manual-QA readiness, User Story 2, migration gate M2 — does not land until after M3, M4, M5 and M6 have all passed. The baseline reading costs a regeneration (which FR-209 requires anyway) rather than a delay. **The question itself remains open**; only its cost is now stated.
- **C4 — SC-004, SC-102, SC-113, SC-115.** Four legitimate human-judgement/observation-based criteria carried no tag saying so, unlike SC-044, which already records its measurement as *"a reviewer's recorded judgement sheet"*. All four now state their observation method and are explicitly marked not automatable, so a future reader does not build an automated gate that cannot measure what the criterion is about. SC-102 is split deliberately: its **diff** half **is** mechanically assertable and its **timing** half is not, and the two must be reported separately so an automated diff pass is never read as evidence about the 30 minutes.
- **F14 — User Scenarios preamble.** *"Forty user stories … unified into 33 — eight merges … and no deletions"* did not close arithmetically (8 merges over 40 sources reads as 32). The reason was already stated in the individual story entries but not in the summary: **`004/US6` contributes to two merged stories** (US26 and US27), so it is folded twice and removed once, making the reduction **seven**, not eight. The half-sentence is added.
- **F13 — Open Questions preamble.** The sentence said *"Eleven … plus two … fifteen total"*, and 11 + 2 is 13. The **11** was the count of **open** rows, silently substituted for the count of **inherited** ones. The sentence is rewritten against the live table, recomputed after every other edit in this pass had landed, and now states each number as what it actually counts. The requirement-count and criterion-count preambles were recomputed for the same reason, since this pass added two requirements and three criteria.

**What this pass did NOT do, stated so it is not inferred.** It resolved no open question. It made
no code change and performed no git operation. It touched no file other than this one. It judged no
content-boundary row and quoted no private material: `workshop`, `ai_interviewing` and
`monetization` are referred to by name and path only, as everywhere else in this document.

---

## Open Questions

**The table below holds seventeen rows, and they arrive from three places.** **Thirteen** were
inherited from the seven source specifications and deduplicated (OQ-1 – OQ-13). **Two** were raised
by the merge itself (OQ-14, OQ-15). **Two** were raised by the 2026-09-15 remediation pass
(OQ-16, OQ-17). 13 + 2 + 2 = **17 total rows**.

Of those seventeen, **four are RESOLVED** — OQ-3, OQ-5, OQ-7 and OQ-9, see Clarifications "OQ-3
resolved during this pass", "OQ-5 resolved during this pass", "OQ-7 resolved during this pass" and
"OQ-9 resolved during this pass"; all four are kept in this table, **not renumbered**, because
FR-036, migration gate M0, FR-243, FR-288, FR-295 and several tasks cite them by number.
**Thirteen remain open.** 4 + 13 = 17.

*(The sentence this paragraph replaces read "Eleven … plus two … fifteen total", which did not
close: **11 was the count of open rows**, silently substituted for the count of inherited ones, and
the table then held fifteen. Recorded rather than quietly corrected, because a count that did not
add up is exactly the kind of error this document elsewhere insists on leaving visible.)*

**None of the thirteen open rows is a clarification marker left in the text**; each is an owned
decision awaiting an operator, and each states why it could not be resolved under the safest-option
rule.
...
| **OQ-7** | **RESOLVED 2026-09-15** (operator decision). Should the standalone `ai_interviewing` (`ai_curriculum`) repository be **PUBLIC** — to satisfy FR-288's "clonable by a user with no access to the umbrella" — or remain **PRIVATE** with invited or credentialed access, given it holds employer information the restricted user is explicitly denied (FR-299)? **Decided**: stays **PRIVATE**, with invited/credentialed collaborator access. FR-288's literal requirement ("no access to the umbrella") is satisfied by direct credentials to this one repository — it does not require public, anyone-can-clone visibility, and going public cannot later be undone while staying private and opening it up later remains possible. | 007/OQ-1 | **No longer applicable** — retained for traceability. This was a real disclosure decision about real private content; investigated for a literal-requirement reading that widened the option space (FR-288 does not in fact force PUBLIC), then put to the operator with no recommended default — the two options carried genuinely different, non-technical costs. |
...
| **OQ-15** | **Raised by this merge.** Must the manual-QA-readiness milestone (User Story 2, migration gate M2) be reached against the **baseline** umbrella-hosted platform before decoupling begins, or against the **target** standalone modules after it? | new | FR-209 requires the QA and client documents to record the build identifier they were written against and to be **regenerated, not patched**, when that build moves — and decoupling moves the build in the largest possible way. Writing them against the baseline and then decoupling means regenerating them; writing them against the target means the operator's stated absolute priority waits on the whole migration. Both are defensible; the choice is a sequencing and priority decision only the operator can make. **Stated consequence, so the cost of each branch is visible before the choice is made: under the written-against-the-target reading, the QA document and the client walkthrough (FR-206 – FR-217) cannot be finalized until the full migration completes — which means the operator's own repeatedly-stated absolute-priority milestone, manual-QA readiness (User Story 2, migration gate M2), does not land until after M3, M4, M5 and M6 have all passed.** Under the written-against-the-baseline reading the milestone lands early and is then regenerated, which FR-209 requires and which costs a regeneration rather than a delay. |
| **OQ-16** | **Raised by the 2026-09-15 remediation pass.** What does "ready" mean in SC-146 and SC-147 — a fresh clone of a standalone module reaching a self-reported ready state in under 10 minutes with a single command — when the shared auth/user store that FR-295 makes a network-consumed external dependency has not yet been provisioned? Specifically: **(a)** does "ready" include reachability of that store, or is a module that starts and correctly fails closed (FR-294, FR-310) against an absent store already "ready"; and **(b)** if reachability is included, does the single documented command **provision** a store, or does it merely **point at** one the operator provisioned beforehand — in which case the bootstrap has a precondition SC-146/SC-147 do not currently state? | new | OQ-9's resolution (2026-09-15) made the user store a separately-hosted component consumed over the network, and FR-294/FR-310 require both modules to deny all access when it is unreachable. A fresh clone with no store provisioned therefore reaches a state that is simultaneously *correctly behaving* and *serving nothing* — and **no artifact in this family states whether that counts as "ready"**. The three candidate answers carry materially different costs: (a)-as-ready makes the criterion cheap but lets a module pass while no user can log in; (a)-with-reachability makes the criterion honest but adds an unstated provisioning precondition to a criterion that advertises a single command; provisioning-in-the-command means each fresh clone stands up its own identity store, which is in tension with the *shared* store OQ-9 chose. This is a product and operations decision about what the 10-minute promise is promising, not an engineering default — and inventing one would be the guess §11.4.6 forbids. |
| **OQ-17** | **Raised by the 2026-09-15 remediation pass.** Which release, or which date, sunsets the transitional dual-field wire state in which `ordinal` and `ordinal_path` are served side by side (FR-036), and at which point is `ordinal` removed entirely? | new | The live server code that places the two fields beside each other scopes this to **"for one release"** in its own comment, but **no artifact anywhere names that release or a date** — not `specs/003-chapter-hierarchy/decision-record.md`, not FR-036, not this document's OQ-3 Clarifications entry, and not the tasks file. The gap has now been flagged three separate times with zero owner, which is why it is promoted to a numbered, owned row rather than recorded as a fourth note. It cannot be resolved here because it is an **operator scheduling decision**, not a technical default: removing `ordinal` breaks every consumer still reading it (front-end `models.ts`, the registry schema, archived artifacts — the exact set the zero-diff decision was chosen to protect), so the date depends on when those consumers are migrated, which is a plan nobody has written. Naming a version or a date here would be exactly the invention §11.4.6 forbids. **Distinct from OQ-3, which is resolved**: OQ-3 settled *what the fields are*; this asks *when one of them stops existing*. |
...
### Remediation-pass criteria — 2026-09-15

*These three criteria were added by the 2026-09-15 remediation pass and have no source-spec origin.
They are numbered last for the same reason SC-163 – SC-165 are: renumbering to place them
topically would break every cross-reference in this document. See Clarifications, "Remediation pass
— 2026-09-15".*

- **SC-166** *(new, derived from remediation of the 2026-09-15 /speckit-analyze report)*: **100%** of destructive operations performed by a completed implementation carry, in their own execution trace or evidence artefact, a preceding hardlinked-backup step and the assertion that confirmed the backup existed before the destructive step ran; destructive operations whose trace shows no preceding backup number **0**. **Measured by** enumerating the destructive operations FR-338 names, driving each one, and reading its evidence artefact for the backup step and its confirming assertion — a code path that merely *mentions* a backup does not satisfy this. Paired mutation: remove the existence assertion from one operation so it proceeds on a backup that was never taken — the check must go red. *(Measures FR-338.)*
- **SC-167** *(new, derived from remediation of the 2026-09-15 /speckit-analyze report)*: **100%** of repositories this work creates or extracts carry all four governance carriers, each opening with the `## INHERITED FROM` pointer heading and all four in lockstep, **and** are declared in a dependency manifest whose recorded ref equals the live gitlink — with both conditions met **before** the repository's first push. Repositories reaching a remote without both number **0**. **Measured by** enumerating every repository the work created, and for each asserting carrier presence, the pointer heading, four-way lockstep, and manifest-versus-gitlink agreement. Paired mutation: push a repository carrying three of the four carriers — the check must go red. *(Measures FR-339.)*
- **SC-168** *(new, derived from remediation of the 2026-09-15 /speckit-analyze report)*: **100%** of authenticated responses from both modules' auth surfaces carry the full header set FR-304 enumerates — `Strict-Transport-Security`, `X-Content-Type-Options: nosniff`, a frame-ancestors control (`X-Frame-Options` or an equivalent `Content-Security-Policy` `frame-ancestors` directive), and a `Content-Security-Policy` — measured on a **live authenticated response**, not on a configuration file. Responses missing any one of them number **0**. Paired mutation: remove one header from the response pipeline — the check must go red. *(Measures FR-304. The measurement is deliberately specified against a served response: a configuration-file assertion is precisely the SOURCE-versus-SERVED confusion FR-266 exists to prevent.)*
...
- **Public internet publication of the curriculum.** The local-and-internal decision stands for the baseline. *(001, 002)* **Target-state note**: FR-288's standalone cloneability is **not** a decision to publish — it is satisfiable with a private repository and credentials. For `ai_interviewing` this is now resolved (OQ-7, 2026-09-15: stays private, credentialed access). For `workshop` the visibility question remains open — OQ-14, additionally blocked on OQ-1.
