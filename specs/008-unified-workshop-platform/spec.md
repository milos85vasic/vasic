...
|---|---|---|
| **M0** | The chapter-`ordinal` type decision and the path-key contradiction are resolved (OQ-3) | Every ordinal collision in the hierarchy follows from one line typing `Chapter.ordinal` as `int`. No implementation resolves it and nothing downstream is implementable until it is settled. |
...
- **FR-036** *(was: 003/FR-022)*: The ordinal derivation MUST NOT return a value that collides between a parent and its child. Its replacement MUST be an ordered path, and the resolution of the ordinal-type question (OQ-3) governs what that path is called on the wire.
...
- **FR-243** *(was: 005/FR-003)*: The documented way to obtain recorded material MUST be in the repository's front-door documentation, and MUST state what a reader gets and what they may not.
...
Fifteen genuinely unresolved items across the seven specifications, deduplicated, plus two raised by
the merge itself. **None of these is a clarification marker left in the text**; each is an owned
...
| **OQ-3** | **BLOCKING.** How is `Chapter.ordinal` typed, and what is the chapter path key? **(a)** widen `ordinal` to an ordered path of integers; **(b)** keep an integer and declare sub-chapters to have no ordinal, making them second-class in every ordering; **(c)** keep an integer and flatten sub-chapters into the same number space, renumbering every existing chapter the first time one is inserted. And separately: the interface contract states the zero-padded ordinal is *"not accepted as a path key"* while the implementation uses exactly that as the path key. | 003 | Both are recorded in specification 001's own artifacts, both are wrong or contradicted today, and the entire chapter-identity model rests on which way each goes. **No amount of implementation work resolves it**, and nothing downstream of migration gate M0 is implementable until it is settled. This is the single highest-leverage open item in the document. |
...
| **OQ-5** | Which external storage backend holds the recorded material once it leaves the repository's object store, and has that backend's own access configuration been verified to keep private material private **before** any pointer or content referencing it is pushed? | 005 | The 2026-09-08 clarification fixed the *mechanism* (Git LFS) but not the *host* or its privacy posture, and the subject repository is itself private. Getting this wrong is a **push-time, irreversible event** of the class this repository already has on record as an incident that could not be undone. "Conservative" here depends on which backend the operator intends to use, and no such choice has been stated. |
...
- A durable place to hold recorded material outside the repository's object store, reachable by the documented step — **the choice of which is OQ-5**. *(005)*
...
| **003 — Chapter Hierarchy** *(2026-09-05, 43 FR, 32 SC, 17 edge cases, 5 stories, 45 tasks)* | The identity spine: the dotted zero-padded id serving five roles unchanged, derived-never-stored hierarchy, byte-lexicographic ordering as a consequence rather than a rule, the equality rule for every chapter-scope comparison, the silent-data-loss defects and their paired mutations, and the hierarchy exposure on existing endpoints. Its blocking `ordinal`-type question is the highest-leverage open item in this document. | FR-015 – FR-054, FR-274, FR-275; US6, US7, US8, US16, US21; EC-008 – EC-023; SC-005 – SC-036; **OQ-3 (blocking)**. |
