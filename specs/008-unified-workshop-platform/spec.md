...
**Fourteen** genuinely unresolved items across the seven specifications, deduplicated, plus two
raised by the merge itself (fifteen total rows below, one — OQ-3 — since **RESOLVED**, see
Clarifications "OQ-3 resolved during this pass"; kept in this table, not renumbered, for
traceability). **None of the fourteen open rows is a clarification marker left in the text**; each
is an owned decision awaiting an operator, and each states why it could not be resolved under the
safest-option rule.
