...
**54 tasks — 50 complete, 4 open.** Sources: 001, 002. Genuinely open: T344 (SC-023 incremental
indexing — real cross-package gap found and proven, fix out of this round's scope, see its own
note), T351 (operator-blocked — no ONNX entailment model on this host), T356 (SC-010 — one real
fabrication-verification improvement kept, two fabrications survive with an understood residual
cause, see its own note), T357 (blocked on T356 plus human per-citation certification, per its own
note — not a pure operator item).
...
- [x] T339 (was: 001/T067) [US3] **[DONE 2026-09-15.** `workshop/docs/search.md` written (203 lines). Documents `baseIndexedKinds` (`pkg/search/service.go:65-67`) and the evidence-check mechanism preventing an advertised-but-empty kind; the media boundary (`corpusMediaReachedVia = "transcripts and captions only"`, `envelope.go:371-378` — no `audio`/`video` kind exists in the vocabulary); and a second, separate filename-shape exclusion (`SourceFilenameShapeExtensions`, `catalog.go:320-323`) found and documented along the way, added after a real 2026-09-02 LAN disclosure of the archive's naming scheme. **The two false clauses identified and corrected, as the task text itself warned existed:** (1) misattribution — the media boundary is caused by `baseIndexedKinds` + the ingestion pipeline, NOT by "Lumen's extension allowlist" (verified against Lumen's actual installed source: the allowlist claim is true of Lumen in isolation, but Lumen is wired only as the `mode=code` search leg, never invoked on media at all); (2) the task's implied "reachable through transcripts settles the media question" is corrected against FR-156 (was FR-016), which literally requires indexing "the recordings themselves" — unmet, and stated as an honest boundary per FR-280 (was FR-031) rather than glossed over, citing `specs/001-workshop-curriculum-platform/analysis.md` finding H3 verbatim. No other file touched.]** Superseded note below, kept for history. **[PATH NOT BUILT — `workshop/docs/search.md` does not exist, and the media boundary is not stated at any other path either; the docs directory holds `faq.md`, `knowledge-model-contract.md`, `limits.md`, `manual.md`, `quickstart.md`, `README.md`, `user-guide.md`, `work-register.md` plus the `prompts/`, `research/`, `training/` and `session-evidence/` trees]** Document the media boundary in `workshop/docs/search.md` — audio and video are reachable THROUGH transcripts, not indexed directly; Lumen's extension allowlist is a compile-time var with no override (FR-031 requires stating what the system cannot do) — **BLOCKER:** none but the work — `docs/search.md` is unwritten. Two clauses of the task text are FALSE and must be corrected rather than transcribed; see the note · **OWNER:** **implementer** — unblocked today (FR-031→unified FR-280, FR-016→unified FR-156)
- [x] T340 (was: 001/T115) [US3] [TDD] **[DONE 2026-09-15.** `workshop/scripts/index.sh` written, wrapping `cmd/index-embed` against a read-only `sqlite3 .backup` copy of the live DB (never opened for writing) — `--chapter`, `--lexical-only`, `--semantic-only`, `--verify-only`, `--swap`/`--no-swap`, `--timeout-ms` (default 5000, passed to `-embed-timeout`) all implemented. **G-CLI-6 real, run against a genuine closed-port TCP refusal** (verified free via `ss -ltn` first, reaching `pkg/embed/embed.go`'s real `classifyText` default/`FailureExhausted` branch — the same code path a genuine full-exhaustion takes): `verify-cli-6-embedding-exhaustion.sh` 7/7 checks pass (exit 2, `reason.code=embedding_backend_exhausted`, live generation provably untouched — fixture db byte-identical sha256 before/after); paired proof `prove-cli-6-embedding-exhaustion.sh` 4/4 (mutates a hidden sibling copy, never the shipped file; real `index.sh` confirmed git-diff-clean throughout). Also exercised the real success path against this host's live ollama (embedded 1/1, exit 0). Registered `G-CLI-6`/`G-CLI-6-proof` (`check-registry-002.tsv`). Re-run independently this session: both gate and proof pass identically. **Honest boundary, stated in the script's own header rather than faked:** no standalone CLI exists anywhere in this module for the lexical build+verify+swap primitive (only caller of `index.Build` is `cmd/workshop-server`'s own boot sequence) — so `--lexical-only`/default perform genuine read-only verification against `generation_members`/`generations`, not a live rebuild; `--swap` reports `swap_performed:false` with an explanatory note rather than duplicating `index-embed`'s own deliberately-manual transplant step (its header: "the transplant back is a deliberate, separate operator step"); this host's `sqlite3` CLI lacks the FTS5 module, so `--verify-only` checks row counts, not full `passages_fts` membership. None of these gaps affect G-CLI-6, which is fully real per the contract.]** Superseded note below, kept for history. **[PATH NOT BUILT — `workshop/scripts/index.sh` does not exist and no equivalent landed anywhere]** Implement the `workshop/scripts/index.sh` wrapper per [contracts/pipeline-cli.md](../001-workshop-curriculum-platform/contracts/pipeline-cli.md) §4.4 over 001/T058 (unified T330)'s generations — `--chapter`, `--lexical-only` (the honest escape hatch when the embedding backend is saturated), `--semantic-only`, `--verify-only`, `--swap`/`--no-swap`, and `--timeout-ms 5000` per call, because an unbounded query is how a 10-minute stall happens. It MUST reserve embedding capacity for interactive queries, which is a hard requirement rather than a nicety: `scripts/ollama-tune.sh` records that with `OLLAMA_NUM_PARALLEL` resolving to 1 a single embed went from 0.74 s to a >90 s client timeout, stalling indexing entirely — *"queue depth is not the defect — serialisation is"* — and without reservation SC-006 fails on every chapter ingest, precisely when people are using the system. Gate **G-CLI-6** (§4.4, §5): point the embedding endpoint at a closed port and assert exit `2` with `reason.code: "embedding_backend_exhausted"` **and** that the previously live generation is still live and serving. **Paired mutation**: map backend failure to exit `1`; the gate MUST go red. The line that must not be blurred: `all embedding servers exhausted` is `2`, never `1` — reporting `1` there makes a broken backend accuse a healthy curriculum — **BLOCKER:** none but the work — `index.sh` is unwritten · **OWNER:** **implementer** — unblocked today (SC-006→unified SC-084, FR-020→unified FR-159, FR-033→unified FR-261, SC-013→unified SC-136, FR-032→unified FR-260, SC-012→unified SC-135)
- [x] T341 (was: 001/T116) [US3] **[DONE 2026-09-15.** `workshop/scripts/crossref.sh` written, following `_common.sh`'s house style (`flock`-based locking, evidence-writer framework, `BASH_SOURCE`-anchored root). `--check-cycles` is fully real: self-reference (X1) and true directed-graph cycle detection (X2, three-colour DFS) over `origin=authored` rows only, via an embedded Python helper matching this module's own `transcribe.sh` precedent for graph arithmetic; `--chapter` scoping real (filters by `passages.scope`). 10 real synthetic-fixture scenarios in `verify-crossref-cli.sh`, all PASS (clean corpus, self-reference, dangling endpoint, authored cycle, embedding-backend-unreachable, registry unreadable/corrupt, chapter-scope in/exclude, rebuild-derived honest gap); paired proof `prove-crossref-cli.sh` 6/6 (3 mutations against the real subject, byte-identical restore verified). Registered `T341-crossref-cli`/`-proof` (`check-registry-002.tsv`). Re-run independently this session: gate and proof both pass identically (171 registered checks). **Honest boundary, stated in the script's own header rather than faked:** `pkg/crossref.Derive` (T332) has NO CLI/HTTP entry point anywhere in this tree — it is invoked exactly once, internally, from `cmd/workshop-server`'s own boot sequence, and building a new `cmd/*` binary for it was outside this task's authorized scope. `--rebuild-derived` therefore runs its full, real preflight (registry readable; embedding backend genuinely curl-probed) and, once preflight passes, reports **exit 2** with `reason.code: derivation_entrypoint_not_built` — never a faked 0, never a silently-skipped rebuild. A second gap: `Derive` takes a whole generation with no per-chapter parameter, so `--chapter` could never be honored for `--rebuild-derived` without extending that package either.]** Superseded note below, kept for history. **[PATH NOT BUILT — `workshop/scripts/crossref.sh` does not exist. The derivation it wraps DOES exist at `workshop/platform/backend/pkg/crossref/` (001/T060 (unified T332), ticked); the CLI over it does not, and no `--rebuild-derived`/`--check-cycles` surface exists anywhere]** Implement the `workshop/scripts/crossref.sh` wrapper per [contracts/pipeline-cli.md](../001-workshop-curriculum-platform/contracts/pipeline-cli.md) §4.5 over 001/T060 (unified T332)'s derivation — `--chapter`, `--min-score`, `--max-per-passage 20`, `--rebuild-derived`, `--check-cycles`. It rebuilds `origin: "derived"` edges for the target generation and **never touches `authored` edges**, which are content rather than derivation ([contracts/passage-contract.md](../001-workshop-curriculum-platform/contracts/passage-contract.md) §7.4 X4); self-references are rejected and traversal is cycle-checked. Exit `1` on a cycle among `authored` edges (a content defect a human introduced), an edge whose endpoint is not in the registry, or a self-reference in authored content; exit `2` when the embedding backend is unavailable for similarity scoring, the registry is unreadable, or the run was interrupted — an unscoreable backend is not a finding about the content — **BLOCKER:** none but the work — the derivation it wraps EXISTS (001/T060 (unified T332)); the CLI over it does not · **OWNER:** **implementer** — unblocked today (FR-018→unified FR-150, FR-033→unified FR-261, SC-013→unified SC-136)
...
- [ ] T344 (was: 001/T127) [US3] [TDD] [REVIEW] Make index rebuild incremental in `workshop/platform/backend/pkg/index/generation.go` (001/T058 (unified T330)): ingesting one new chapter MUST index only that chapter's passages and MUST NOT re-embed or rewrite the FTS5/semantic entries of chapters already indexed. Gate **G-IDX-1**, proving SC-023: measure index-build wall time for a chapter added when one chapter already exists, then for one added when several exist, and assert the second is not larger than the first beyond measurement noise. **Paired mutation**: force a full-corpus re-embed on every ingest; the gate MUST go red once build time grows with prior chapter count — **BLOCKER:** **real, measured, and now proven structurally — not host capability, a genuine unfixed gap two packages outside generation.go's ownership** — investigated 2026-09-15: the lexical leg's full-rewrite-per-Build is a THIRD-PARTY (`submodules/passage`) design choice, cheap and not the real cost. The real SC-023 violation: `pkg/search.CarryForwardVectors`/`pkg/crossref.CarryForwardCrossrefs` require the source and target generations to share an IDENTICAL `root_hash`, and `root_hash` covers the WHOLE member set, so adding one chapter always changes it — carry-forward then copies NOTHING and every passage, old and new, is reported pending for a fresh embed. Proven structurally (not by flaky wall-clock at unit scale) by new `pkg/index/incremental_test.go`'s `TestGateIDX2_ChapterAdditionInvalidatesCarryForwardForTheWholeCorpus` — 3-passage generation + 2-passage addition -> `Copied==0, Missing==5` (all 5, not just the 2 new), with a positive control proving the mechanism DOES work for its intended identical-root_hash case. Registered as `G-IDX-2` (G-IDX-1 was already claimed, unrelatedly, by generation.go's own verify-before-swap gate — collision avoided by grep before naming) plus `debt` row `sc023-incremental-embedding-not-implemented` in `check-registry-002.tsv`, both re-confirmed passing this session. The real fix — matching a carry-forward candidate per-pid by `(pid, content_hash)` instead of whole-generation `root_hash` equality — lives in `pkg/search/carryforward.go` + `pkg/crossref/carryforward.go`, both outside `generation.go`'s exclusive ownership this round; a cross-package change to shared indexing paths, correctly left for a dedicated follow-up rather than rushed · **OWNER:** **implementer**, next round, owning both carryforward.go files (FR-058→unified FR-160, SC-023→unified SC-092)
- [x] T345 (was: 001/T128) [US3] [TDD] Prove SC-022: with at least 5 concurrent simulated clients issuing `/api/suggest` and `/api/search` requests against a running instance, assert the SC-005 (200 ms p95 suggestions) and SC-006 (2 s p95 results) budgets T053/T065 already measure single-user still hold under that load. Extend `workshop/scripts/bench-suggest.sh`/`bench-search.sh` (001/T065 (unified T337)) with a `--concurrency N` mode rather than a second harness. **Paired mutation**: run the same load against a build with no connection-pool/queue limit; the gate MUST go red once queue contention pushes p95 over budget — reproducing the 20.16 s / 11.05 s / 0.10 s embed-call spread already measured under load (Edge Cases) (FR-055→unified FR-062, SC-022→unified SC-165)

      **DONE 2026-09-15, harness real; SC-022 itself MEASURED NOT MET for `/api/suggest` — same "harness done, criterion unmet are different statements" precedent as T343/T337.** Investigated first: `bench-suggest.sh`/`bench-search.sh` don't exist (confirmed, matching T337's own note) — both endpoints live in one Go binary, `cmd/bench/main.go`. Added `-concurrency N` there (default 1, byte-identical to prior single-goroutine measurement; N>1 runs N goroutines concurrently, merges all samples into one p50/p95/p99 population, adds a `concurrency` report field). New `verify-search-concurrency.sh` (SC-022-concurrency) runs both legs at concurrency>=5 with FAIL-outranks-UNDET precedence; paired proof `prove-search-concurrency.sh` (a throwaway local stub server, never the live deployment, per this repo's `verify-search-latency.sh` precedent) — 8/8 mutations caught. Registered in `check-registry-002.tsv`. Re-run independently this session: gate exit 1 (real, not a bug) — `suggest p95=250.9ms vs 200ms budget FAIL`, `search p95=42.6ms vs 2000ms budget PASS`; proof 8/8 caught, exit 0. **Real measured figures, live deployment**: suggest p95 121.4ms (concurrency=1) -> 251.0ms (concurrency=5, BREACHES); search p95 24.8ms -> 56.7ms (concurrency=5, holds). **SC-022 is NOT MET for `/api/suggest` under 5-way concurrency** — a genuine, reproducible finding, not synthesized; recorded here rather than the budget being quietly widened to clear it.
...
- [ ] T356 (was: 001/T078) [US4] [TDD] **[STILL OPEN 2026-09-15 — Part A (flag surface) DONE; Part B (fix the fabrication) genuinely attempted, one real safe improvement kept, SC-010 remains honestly UNMET.** Added a real long-option `--questions`/`--report`/`--min-answerable`/`--timeout`/`--base` parser to `bench-answers.sh` (env vars still work unchanged as documented fallback; `--answerable`/`--unanswerable` are aliases onto `--questions` per quickstart.md's own already-withdrawn two-file contract — conflicting paths correctly rc 2, not silently picked). Verified safe: no-flag behavior byte-identical, existing `prove-bench-uncited-mutation.sh` still 4/4. **Part B, fresh measurement (against a snapshot-backed standalone instance, since the shared live container runs `-answer-provider none` by deliberate compose.yml default and was never touched):** 24A/33U corpus, `answered+cited 4 / declined 20`, `refused 29 / FABRICATED 2 / unavailable 2`. The single 2026-09-03 fabrication is now correctly declined (corpus growth since then), but two NEW ones appeared. Root-caused #2 ("annual subscription cost") by direct reproduction: `LexicalSupport.Verify` concatenated ALL of a claim's citations into one haystack before scoring, letting a claim launder a missing word via an irrelevant extra citation — inconsistent with `EntailmentSupport`'s own per-citation semantics. **Fixed** (rewrote `Verify` to score each citation independently, matching that precedent) and verified safe: `go build`/`vet`/`gofmt` clean, `go test`/`go test -race ./pkg/answer/...` clean, `prove-mutations.sh` 24/0/4-skip (skips pre-existing and unrelated). **The fix did NOT eliminate either fabrication** — re-measured after: same `answered+cited 5`, same 2 FABRICATED. Both surviving cases clear the lexical floor on a SINGLE citation alone (no cross-citation padding needed), so the union-vs-per-citation distinction never applied to them; the real residual defect is a single fabricated/echoed word riding inside an otherwise well-grounded claim (one case: the word "annually", copied from the QUESTION's own wording, not from any citation) — too small for any coarse word-overlap floor to catch without risking false rejections across the answerable set, which was correctly not attempted. L4 (real entailment, which could plausibly catch this) remains confirmed unavailable on this host (T351: no ONNX model). **Bonus finding, unfixed, out of authorized scope**: the "memory megabytes" case is NON-DETERMINISTIC across identical repeated requests — traced to `pkg/answerhood/ollama.go`'s L5 judge call setting `temperature:0` but NO fixed seed (unlike the main generator, which sets both `temperature:0` and `seed:7`) — meaning SC-010's fabrication count is not currently a stable property of this deployment even on an unchanged corpus. `pipeline.go` was read but confirmed unedited (T362's disconnect-handling block already present and coherent). **SC-010 remains NOT MET** with a real, now well-understood residual cause, rather than bluffed as fixed or left unexplained — matching this file's own established "measured, not met" convention]** Superseded note below, kept for history. **[PATH CORRECTED 2026-09-02 — this task named `workshop/scripts/bench-answers.sh`, which does not exist; the harness landed at `workshop/platform/backend/gates/bench-answers.sh` and **has run**. It is three-valued and class-aware, checks SC-009's ≥20 floor before the first model call, and prints top score and margin on every response including successes. Its configuration is by ENVIRONMENT (`QUESTIONS`, `REPORT`, `MIN_ANSWERABLE`, `TIMEOUT`) plus one positional base URL — **not** the `--answerable`/`--unanswerable`/`--report` flags this task contracts, which is a real divergence and is the flag surface still owed. **Do not rebuild the harness.** What is open is SC-010's verdict, which is measured and still NEGATIVE — **but the "3 fabrications" figure this note carried is WITHDRAWN, not restated.** ~~`bench-expanded-2026-09-01.tsv:18`–`:19` records 30 refused, 3 FABRICATED, `SC-010 NOT met (3 fabrications)`.~~ Superseded twice on 2026-09-02 by the A/B run recorded at `workshop/platform/backend/evidence/answering/bench-question-verifier-ab-2026-09-02.tsv`, and the sequence matters more than either number. With the 001/T115 (unified T340) answer-against-question layer **unwired**, that file's `:24`–`:25` put the unanswerable set at 22 refused and **11** fabricated; with it **wired**, `:27`–`:28` put the same set at 29 refused, **1** fabricated and 3 unavailable. Both runs record SC-010 as NOT met. **The old 3 was therefore not an improvement on 11 — it was taken over a smaller corpus**, which that file explains at `:31`–`:33`. Honest denominator, stated at `:42`: **1 fabrication in 30 MEASURED**, with the 3 unavailable excluded rather than scored as refusals. The surviving case is isolated at `:53` and identified in the per-question rows. **SC-010 is NOT met**, and — see standing rule 7 — this is **spec 001's** SC-010, not spec 002's. Fix the surviving fabrication, then re-run. — **BLOCKER:** none but the work — the harness landed at a corrected path and HAS run · **OWNER:** **implementer** — unblocked today (SC-010→unified SC-094, FR-022→unified FR-166, FR-033→unified FR-261)
...
- [x] T361 (was: 001/T083) [US4] **[DONE 2026-09-15. `workshop/docs/answering.md` written (176 lines), reusing existing content per the task's own instruction rather than rewriting it.** Documents the async contract (`GET/POST /api/ask` -> HTTP 202 + job, verified at `pkg/answer/http.go:33-39`); the `estimated_seconds: null` requirement, quoted verbatim from `faq.md:215-216`/`quickstart.md:388-389` rather than re-derived; real measured latency cited from `docs/limits.md` §1.4 (70.6s/30.5s same-question runs, 2.3x spread, 11-67s benchmark band). The 21s/95s research-estimate figures were traced to their real source (`specs/001-workshop-curriculum-platform/research/llm-bridging.md:678`) and found explicitly labeled `UNVERIFIED — no generative model is installed` in that source — the page states this distinction (estimate vs. live measurement) explicitly per anti-bluff discipline rather than presenting the old estimate as measured fact, and notes the installed 3B model's real measured range (30.5-70.6s) is broadly consistent with it. Cross-references `docs/limits.md` §§1.5/4.1-4.2/9 and declares `limits.md` authoritative on any disagreement, per that document's own stated rule. No other file touched. Superseded note below, kept for history.]** Document in `workshop/docs/answering.md` that answering is ASYNCHRONOUS — CPU-only generation is ~21 s (1.5B) to ~95 s (7B) idle, so "instant" is off by two orders of magnitude and no prompt engineering closes it. `estimated_seconds` MUST be `null` until measured. Superseded pre-2026-09-15 note: **[PATH NOT BUILT — `workshop/docs/answering.md` does not exist. Part of the content landed at OTHER paths and should be reused rather than rewritten: the `estimated_seconds: null` contract is documented at `workshop/docs/faq.md:215`–`:216` and `workshop/docs/quickstart.md:388`–`:389`, and the asynchronous-answering timing story is in `workshop/docs/limits.md` §1.4. The PAGE this task contracts still does not exist]** Document in `workshop/docs/answering.md` that answering is ASYNCHRONOUS — CPU-only generation is ~21 s (1.5B) to ~95 s (7B) idle, so "instant" is off by two orders of magnitude and no prompt engineering closes it. `estimated_seconds` MUST be `null` until measured — **BLOCKER:** none but the work — `docs/answering.md` is unwritten; part of the content already landed at other paths and should be reused, not rewritten · **OWNER:** **implementer** — unblocked today (FR-031→unified FR-280, FR-030→unified FR-336)
- [x] T362 (was: 001/T129) [US4] [TDD] [REVIEW] Discard partial answer output on mid-generation failure or disconnect in `workshop/platform/backend/pkg/answer/pipeline.go`: if the provider stream (001/T068 (unified T346)) errors or the connection drops after tokens have started streaming, the handler MUST NOT surface any of the partial text as the answer — it reports a failure, matching the "refuse the whole answer" discipline 001/T074 (unified T352) already applies to a failed grounding layer. Gate: kill the provider stream mid-response and assert the client receives a failure response, never a truncated-but-displayed answer. **Paired mutation**: return whatever text had streamed so far as the answer on disconnect; the gate MUST go red (FR-053→unified FR-170)
      ...
      **DONE 2026-09-15.** Investigation found `pipeline.go`'s `Ask()` already assembled `Outcome.Text`/`Citations` only from the atomic `res.Claims` at the very end, never incrementally, and the provider-error half was already correct (`ollama.go` discards its buffer and returns a `*Reason` on stream error; `pipeline.go` already discarded `res` unconditionally whenever `reason != nil`). **The real gap**: nothing checked `ctx.Err()` after `Provider.Generate` returned SUCCESSFULLY (`reason == nil`) — a provider is free to finish decoding a response it already had and hand back a "clean" result even after the caller's context ended (client disconnect/deadline), and nothing defended against trusting that stale-but-clean result. Fix: a `ctx.Err()` check immediately after `Generate` returns with no reason, discarding `res` and returning `unavailable(&Reason{Code: CodeCancelled, ...})` — reuses the existing `CodeCancelled` vocabulary, no new wire shape. TDD real RED (`the client disconnected mid-stream and still received an answered outcome ... a truncated-but-displayed answer, exactly what T362 forbids`) -> GREEN (`TestAsk_ProviderStreamErrorMidGeneration_DiscardsPartialText` + `TestAsk_ProviderReturnsResultAfterContextCancelled_DiscardsPartialText` both PASS) -> real mutation (removed the check, confirmed the exact RED message returns, restored, rebuilt clean). `TestGate`-style, no separate shell gate, matching this package's own convention for TDD-only gates. Re-run independently this session as part of the full `go build/vet/test ./...`: 24+ packages green, including `pkg/answer` under `-race`.
...
- [x] T369 (was: 002/T149) [US3] [TDD] [REVIEW] **[DONE 2026-09-15 — already structurally satisfied; no production fix needed, confirmed by real evidence rather than assumed.** Gate id renumbered: the task's own text names `G-KG-23`, already claimed by Phase 8's T291 — grepped fresh, highest was G-KG-24, used **G-KG-25**. Investigation: `pkg/search/offsets.go`/`lexical.go` carry NO cache of their own — every snippet/offset is derived at request time from a live FTS5 query (`Lexical.Retrieve`), so the only place staleness could live is one layer down, in whether `passages_fts` itself updates synchronously with a redaction. It does: `internal/redaction.Plan.Apply` calls `passage.Registry.BuildDB` (the same call `cmd/workshop-redact` makes as one redaction run), which deletes+reinserts every row in one transaction, skipping `passages_fts` entirely for any redacted row. New `pkg/search/g_kg_25_test.go`: builds a real derived DB with a synthetic passage, confirms a real match with offsets, redacts it via a real `Plan.Apply` with NO reopen/reload, re-queries the SAME open DB/`Lexical`, asserts nothing survives — PASS on first real-code run (confirms "already correct", the task's own explicitly allowed outcome). Second test is the oracle-meaningfulness control: skips `BuildDB` (reproducing a real 2026-09-03 incident recorded in `docs/work-register.md` §6.8, which was an OPERATIONAL gap — a redact run whose derived-DB rebuild step was simply never invoked against that deployment's file — not a design defect) and confirms the stale representation DOES leak, proving the first test isn't vacuous. `verify-g-kg-25-search-redaction-sync.sh` + paired proof (swaps the real subject call for a type-compatible no-op, sha256-restored) both re-run independently this session: PASS / 4-for-4. Full 24-package suite green.]** Superseded task text below, unchanged: Extend redaction propagation (002/T021 (unified T059), FR-027→unified FR-148) to every cached or
...
- [x] T374 (was: 002/T067) [US3] [TDD] **[DONE 2026-09-15 — fresh measurement supersedes the stale figure below; SC-015 now MET.** Re-ran `verify-retrieval-benchmark.sh` fresh against the live deployment (generation 13): **PASS 5/5 — top-5 21/22 (95.5%), top-1 20/22**, against the 20/22 bar; per kind: area 4/5, question 5/5, **term 12/12 (100%, every one ranked #1)** — directly contradicting the stale top-5 12/22 figure below, which the assignment itself warned might be superseded by corpus/code movement. Root-caused rather than assumed: (1) `pkg/search/semantic.go`'s own commit history (2026-09-04, before this session) already fixed the historical pollution — `kg_term` used to compete unfiltered in the semantic leg (nonsense controls scored 0.60-0.65, indistinguishable from real matches); `Semantic.WithPopulation()` now excludes it entirely. (2) Today's clean measurement is ALSO because production currently serves lexical-only by deliberate, DOCUMENTED, measured operator decision (`compose.yml` commit 807f2c1, 2026-09-09: model-on vs model-off scored IDENTICALLY on this same 22-query benchmark at generation 7, 9.4x slower with the model) — not an accident. (3) One plausible narrow enrichment was tried (appending a term's parent-area title to its indexed `Body`) and empirically measured to do NOTHING on real data — `pipeline/extract/taxonomy.py` hard-codes `member_terms=None` for every "promoted" area, and all 42 live areas are promoted — so the change was reverted rather than shipped as dead code (`git diff --stat` confirmed clean). `pkg/search/` is byte-identical to session start. Re-ran both the gate and its paired proof (`prove-retrieval-benchmark.sh`, 11/11 mutations caught) a second time as the final step — identical, byte-for-byte. **One open, honest caveat, not a regression**: this PASS has not been (and could not safely be, within this task's scope) re-verified with the semantic leg genuinely active on a fully-embedded generation-13 corpus — embeddings on this deployment currently exist only through generation 12. Low-risk given population-scoping already excludes `kg_term` from that leg by design, but not zero-risk; worth a follow-up if/when semantic is re-enabled. Full 24-package suite green throughout.]** Superseded task text below, unchanged: Prove **SC-015**: the gate prints **per-query** outcomes, not only the — **BLOCKER:** **retrieval quality, and nobody's decision** — the instrument is built and correct; SC-015 is MEASURED NOT MET at top-5 12/22 (54.5%), top-1 3/22, against a bar of 20/22. The `term` leg is the shortfall · **OWNER:** **implementer** — unblocked, but it is a quality problem, not a wiring problem
...
- [x] T375 (was: 002/T068) [US3] [TDD] **[DONE 2026-09-15 — SC-016/SC-017 proven via a clean, watched re-run; the historical crash's root cause is PLAUSIBLE, not CONFIRMED, and this is a genuine, structural evidentiary limit rather than insufficient investigation.** Checked `podman inspect` first (rather than assuming): `RestartCount:0`, `OOMKilled:false`, no memory limit configured on the service (`HostConfig.Memory=0`, nothing in `compose.yml`), `podman logs --since 240h` grepped for panic/fatal/OOM/killed/signal/goroutine — zero matches. **The 2026-09-03/04 incident itself is forensically unrecoverable, structurally**: the host journal's oldest entry is 2026-09-09 (predates the incident), and the container instance that crashed then has since been fully recreated — neither its logs nor the journal from that period exist on this host to compare against. Cautious re-run of the EXACT harness (`verify-search-latency.sh`) against the live deployment, watched throughout via a parallel `podman inspect`+`/api/health` poller: container never restarted, stayed healthy the entire time; 5 of 7 legs PASS cleanly (search fused/lexical, suggest, health CONTROL, resolve-citation all comfortably under budget); `search semantic`/`search code` UNDET, explained deterministically (this container instance runs with no `-ollama` endpoint configured, a separate, already-known finding — `search-floor-uncalibrated` in `defects-registry.tsv` — not a crash). Code review of `pkg/search/semantic.go`/`lexical.go`/`internal/api/passages.go`: no goroutine spawned per search/passage request in either code path; the one long-lived subprocess in this codebase (`pkg/entail/onnx.go`) is on the `/api/ask` verification path, unrelated. **Independent corroborating evidence emerged later the same session**: the shared container DID exit (rc 137, SIGKILL) once during this Phase's parallel work, at the same moment several entirely UNRELATED host containers (qbittorrent, jackett, postgres/redis sidecars) also died with the identical signal — consistent with host-level memory pressure across this multi-agent host under this session's heavy concurrent load, not a workshop-server-specific defect; restarted cleanly via the module's own idempotent `scripts/start.sh` by a concurrent track (T374), confirmed healthy at the same build afterward. **Honest verdict**: SC-016/SC-017 ARE proven (clean, watched run, no crash); the ORIGINAL 2026-09-03/04 root cause remains genuinely UNCONFIRMED — not asserted as closed, because the evidence to confirm it no longer exists — while the PLAUSIBLE explanation (shared-host memory pressure under concurrent load, not a code defect) is now independently corroborated rather than merely inferred.]** Superseded task text below, unchanged: Prove **SC-016 / SC-017**: re-run the latency harness and **publish before and — **BLOCKER:** **NOT "just a quiet host" — that remedy is now IN DOUBT, and this is the correction that matters.** Re-run 2026-09-04: rc **2**, `PASS 0 FAIL 0 UNDET 7`. The gate read the index block cleanly (generation 68, 12,979 passages, live) and then **every** endpoint refused — including the `health CONTROL`, which by the harness's own design makes the fused figure uninterpretable. Immediately afterwards `podman ps` showed `workshop-curriculum_platform_1  Up 46 seconds (starting)` and `/api/health` returned `HTTP=000`: **the container went down underneath a read-only run.** `tasks.md` already records the same sequence on 2026-09-03, so counting this it has happened **three times across two dates, every time during or immediately after `verify-search-latency.sh`**. Host load was FALLING across this run (17.5 -> 9.7), which weakens the load-average explanation the old note leans on. No causal claim is made — co-occurrence is not causation and this was not isolated — but the narrow, checkable claim is: the stated remedy *"one clean re-run, not new code"* has now failed three times identically, so treating this as bad luck risks a fourth identical result · **OWNER:** **implementer first** — find why the container dies under this harness's concurrent request pattern; that outranks the latency figure. **operator** second, for a quiet host, once the stability question is settled
...
**26 tasks — 26 complete, 0 open.** Sources: 002, 004, 005.
...
- [x] T408 (was: 004/T081) [P] [US5] Convey per-question correctness in the test result panel by an icon or text label in addition to colour, in `workshop/platform/frontend/src/app/features/areas/area-test.component.ts` (FR-022a→unified FR-228). A structural check over the served bundle MUST confirm every per-question result carries a non-colour indicator, in both light and dark presentation.

      **DONE 2026-09-16.** Investigation found the ONLY correctness signals were an `aria-hidden="true"`
      tick/cross glyph (invisible to assistive tech) and a border colour — the template's outcome
      branch had NO case at all for a graded/answered/CORRECT outcome, so a correct answer rendered
      zero distinguishing text. Fix: a visible `.out__badge` span (`outcomeLabel()`: "Correct" /
      "Incorrect" / "Self-review") added alongside the existing icon — additive, never replacing the
      colour cue — using the codebase's existing CSS-custom-property light/dark pattern. TDD real RED
      (`git stash` the component: 3/8 new tests fail with the expected "label text absent" messages) ->
      GREEN (8/8). Full Karma suite: 393/393 SUCCESS. New structural gate, driving a real headless
      browser against the SERVED bundle (never the source, per T410's own D6 discipline):
      `platform/gates/sample-served-test-outcomes.mjs` + `verify-served-correctness-indicator.sh` +
      paired `prove-served-correctness-indicator.sh` (8/8 mutations caught, including a
      non-vacuity control that an aria-hidden-but-visually-painted label correctly does NOT trip the
      gate, since 1.4.1 is a visual criterion). Registered `T408-non-color-correctness-indicator`/
      `-proof`. **Honest live-bundle result, re-confirmed independently this session**: run against
      the live deployment, the gate correctly finds 4 real findings (one per row/scheme) — the
      container is still serving the pre-fix bundle, since a `platform/web/` rebuild+restage is a
      separate operational decision this session has consistently deferred (see Phase 8/9's own
      precedent). The gate and its proof are both real and correct; only the deploy step remains.
- [x] T409 (was: 004/T049) [US5] Emit `corpus_revision` on the served catalogue and cite it in every interface claim (FR-023a→unified FR-230, SC-018→unified SC-042). **Analysis finding F7 — MISPLACED**: this is a catalogue concern owned by the backend stream, not a presentation one. Execute it with Phase 3 (US1); it is listed here only because the superspec blueprint's Track A carries a *bundle* build id, which is a different artefact, and this task must not be assumed covered by it.

      **DONE 2026-09-16.** Confirmed genuinely unbuilt first (`corpus_revision` grepped nowhere in
      `workshop/` or this tasks.md before this task) and confirmed Phase 3 (US1) never covered it —
      the nearby "build id" hits are the search-index fingerprint (`/api/health`'s
      `build`/`source_commit`) and `/api/index/status`'s `generation`/`root_hash`, a different
      artefact, exactly as this task's own warning says not to conflate. Reused rather than invented
      (§11.4.74): `index.Generation.RootHash` (a sha256 content hash over every non-redacted
      passage's `(pid, content_hash)`, already stable-when-unchanged/changing-when-changed by
      construction) is the one already-computed identity worth serving, since the catalogue's own
      `evidence_pids` resolve against this exact passage registry. New `CorpusRevisionSource`
      type/adapter (`internal/api/router.go`), wired via `KnowledgeDeps.CorpusRevision` in
      `cmd/workshop-server/main.go`; `GET /api/areas` and `GET /api/areas/{area}` (`areas.go`) now
      emit `"corpus_revision"` at the envelope level — nil/unwired degrades to JSON `null`, never a
      fabricated string, mirroring the existing `liveGen` discipline. Frontend: `area-catalogue.ts`
      captures it into a signal; `area-detail.component.ts`'s `cardFacts` block (the one interface
      claim on this page genuinely wired to the real, tested backend contract) now cites it —
      deliberately NOT the speculative `'ready'`/`passage_count` branch, which reads a model marked
      "ASSUMED" in its own header and does not match the real measured response shape. TDD real RED
      (stashed the 3 backend files: `undefined: api.LiveCorpusRevision`; stashed the 2 frontend
      files: `Expected null not to be null`) -> GREEN on both. Full suites: backend 24+ packages
      green; Karma 393/393 SUCCESS.
...
- [x] T413 (was: 005/T046) [US4] **DONE 2026-09-16 — a fresh served run WAS captured this session.** `WORKSHOP_BASE_URL=http://192.168.1.115:8087 bash platform/gates/verify-served-palette.sh` -> exit **0**: **204 surfaces graded (0 unusable), 128 carrying an own painted colour, 7 distinct hue families against the floor of 6** (families 000-030, 030-060, 120-150, 180-210, 300-330, 330-360, plus neutral, each naming its own real surfaces — e.g. `tag`, `button-rule`, `heading`, `link`). Re-confirmed the instrument itself is trustworthy: `prove-served-palette.sh` re-run, 8/8 mutations caught (rc 0), gate source byte-identical across the battery. This supersedes both the CONTINUATION.md:646 baseline of 2 AND the earlier "no served run captured" state — the count is now measured, not inherited, and clears the floor with one family to spare (FR-016→unified FR-224, SC-012→unified SC-116).
- [x] T414 (was: 005/T047) [US4] **DONE 2026-09-16** — every hue present now traces to a stated classification job, so a hue cannot be added only to raise a count (FR-017→unified FR-225, SC-012→unified SC-116). **What already existed**: `workshop/docs/design-system/TOKENS.md` section "2.5 The complete hue map" (commit `f11a3c1`) already names a job for every declared hue — 0° fault, 32° warning, 123° good, 208° HEARD & SEEN, 218° (~0% chroma) CUSTODY, 252° WRITTEN, 300° BUILT, 354° the brand's own voice — with a "never used for" column for four of them. **What was missing, and is now built**: `workshop/platform/gates/verify-served-palette-jobs.sh` + paired `prove-served-palette-jobs.sh`. The gate parses TOKENS.md's own §2.5 table directly (never a private copy), buckets each declared hue into the same 30° families `verify-served-palette.sh` already uses, takes the same live-or-fixture sample, and requires every NON-NEUTRAL hue family the sample actually paints to fall in a bucket the table declares a job for — a family with none is the FINDING. Deliberately bucket-level rather than a literal CSS-custom-property trace (documented honest boundary in the gate's own header: getComputedStyle resolves after cascade/compositing, so exact-hex tracing would false-positive on legitimate declared colour). **Evidence, this session**: `WORKSHOP_BASE_URL=http://192.168.1.115:8087 bash platform/gates/verify-served-palette-jobs.sh` → exit 0, 6 non-neutral hue families painted on the live deployment, every one traced to a declared TOKENS.md job (fault/warning/good/heard/built/accent — written (252°) is declared but not currently painted, which is fine: the requirement is every PAINTED hue has a job, not every job is painted). `prove-served-palette-jobs.sh` → 8/8 mutations caught (M1 is the load-bearing one: a sample with one extra surface at hue 60° — bucket 060-090, which TOKENS.md's table names no job for — turns the gate red; M2 confirms an all-neutral sample is not itself a finding; M3–M7 cover vacuity and four ways the hue-map document can fail to parse, all UNDETERMINED not clean). Registered `G-SERVED-PALETTE-JOBS` / `-proof` in `check-registry-002.tsv`. Confirmed no regression: `verify-served-palette.sh` / `prove-served-palette.sh` unchanged (7 families / floor 6, 8/8 mutations, gate source byte-identical to before this task).
...
- [x] T416 (was: 005/T049) [US4] **DONE 2026-09-16 — a fresh served run WAS captured this session.** `WORKSHOP_BASE_URL=http://192.168.1.115:8087 bash platform/gates/verify-served-contrast.sh` -> exit **0**: **196 pairings graded (8 unusable, not counted as passes), 0 failures — every pairing clears its floor, in each scheme (light/dark) independently**, both text (4.5:1 floor) and non-text (3.0:1 floor). Re-confirmed the instrument itself: `prove-served-contrast.sh` re-run, 6/6 mutations caught (rc 0), gate source byte-identical across the battery. Result recorded here rather than left uncaptured (FR-019→unified FR-227, SC-013→unified SC-117).
- [x] T417 (was: 005/T050) [US4] **DONE 2026-09-16** — a future population change now MUST carry a recorded, independent justification, and a mechanical gate checks that on every run, not just at diff time (FR-018→unified FR-226, SC-012→unified SC-116). **Mechanism chosen, and why**: the sampler's admitted-selector population (`SURFACES`) was extracted verbatim out of `sample-served-surfaces.mjs` into its own side-effect-free module, `workshop/platform/gates/surfaces-manifest.mjs` — so there is exactly ONE definition of the population, imported rather than re-declared, and it cannot drift from what the sampler actually samples. `workshop/platform/gates/POPULATION-CHANGES.md` is a new **append-only changelog**: every entry names the sha256 of `surfaces-manifest.mjs` at the time plus a mandatory non-empty `Reason:` line. This mirrors this codebase's own existing changelog-discipline convention rather than inventing one — `platform/gates/defects-registry.tsv` cross-checked against `docs/limits.md` by `grep -F` anchor, and (more precisely, since what's being guarded is "does the live artifact still match" rather than "does the doc still mention X") the umbrella's own `helix-deps.yaml` `deps[].ref`-must-equal-live-gitlink pattern (cascade check C9). `workshop/platform/gates/verify-population-manifest.sh` + paired `prove-population-manifest.sh` is the genuinely mechanical enforcement (a script that FAILS, not a written policy): it hashes the live manifest, requires it to equal the changelog's last entry's recorded hash (a mismatch = an undocumented population change — exactly "a red fixed by making the instrument weaker" with no matching entry), requires every changelog entry to carry a Reason, and requires every live `SURFACES` row's `why` to be non-empty and above a minimum length. **Evidence, this session**: `bash platform/gates/verify-population-manifest.sh` → exit 0 (live population hash matches the changelog's one recorded entry, which documents the mechanism's own creation as a non-population-changing split — verified by `verify-served-palette.sh` reporting the identical 7 families / 204 surfaces on the live deployment before and after the split). `prove-population-manifest.sh` → 8/8 mutations caught (M1 is the load-bearing one: a selector appended to a scratch copy of the population with the real changelog left untouched turns the gate red; M2/M3 catch a changelog entry with no Reason and a blanked `why`; M4–M6 cover vacuity, an unparseable entry, and missing files). Registered `G-POPULATION-MANIFEST` / `-proof` in `check-registry-002.tsv`. Confirmed no regression: `verify-served-palette.sh` / `prove-served-palette.sh` and `verify-served-contrast.sh` (both consumers of the refactored `sample-served-surfaces.mjs`) are unchanged (7 families / floor 6 and exit 0 respectively, both provers 8/8 and 6/6, gate sources byte-identical).
...
**11 tasks — 11 complete, 0 open.** Sources: 005, 006. Two genuine, non-blocking findings recorded rather than fixed (out of scope for this phase): a pre-existing test-isolation gap in a different gate's probe (see T477), and G-CH-3/G-CH-5's own scanroot path bug (see T470).
...
- [x] T469 (was: 005/T053) [TDD] **PARTIAL** — make every check whose subject is a set establish the set is **non-empty** before reporting the set contains no defects (FR-022→unified FR-262, SC-014→unified SC-135). Two instruments do this: `verify-obtainability.sh` returns **2** when nothing is tracked to measure, and `workshop/platform/gates/verify-bank-reachability.sh` was written precisely because the prior gate checked one fixture area while 89 questions were unreachable. **It is not universal, and no sweep asserts it across the fleet.**
      **[DONE 2026-09-16] The stated gap — "no sweep asserts it across the fleet" — is closed: a real, working sweep now exists, and it was iterated on real fleet data rather than accepted on its first honest-but-flawed run.** New `platform/gates/verify-vacuous-truth-sweep.sh` runs every registered `check` row across all four `check-registry-*.tsv` files (glob-discovered, no hardcoded roster) and classifies each clean run SAFE (population evidence found) / EXEMPT (declared in new `.vacuous-truth-allow`) / FLAGGED (no evidence, weak signal) / FAIL (the sharp defect: every population hit in the output reads zero AND the check still exits 0). `--prove-failure`: **6/6** real mutations, independently re-verified multiple times across this session.
      **The first real-fleet run was itself the honest finding this task's own text describes wanting.** It reported FAIL=27, and rather than trust that number, each of a sample of 7 was manually re-run — all 7 were false positives of the detector's own ±15-character cross-context window (e.g. `verify-bank-reachability.sh` itself, this task's own flagship non-vacuous example, misfired because its healthy `unknown-area 0` sub-metric sat textually near the population word "area"). **This was reported, not hidden, and then fixed rather than left as a known limitation**, since the root causes were precise and well-scoped:
      1. **ANY-zero → ALL-zero.** A check reporting real non-zero population evidence ANYWHERE, alongside an unrelated zero-valued health sub-metric, is not vacuous — FAIL now requires every population hit in the output to be zero. Fixed `verify-bank-reachability.sh`'s false positive.
      2. **Same-line only, never cross-line.** A digit on one line must never pair with a population word on an adjacent line.
      3. **`key=N` status fields excluded.** `rc=0`/`exit=1` are exit-code fields, never population counts in this codebase's own house style. Fixed `prove-floor-domain.sh`'s false positive ("...the domain matches rc=0..." was misread as "0 matches").
      4. **Standalone digit tokens only** (`\b\d+\b`), never a digit embedded inside a random alphanumeric identifier (a tmpdir suffix, a hash fragment). Fixed `verify-cli-6-embedding-exhaustion.sh`'s false positive (an incidental digit in `/tmp/verify-cli-6.k6eGJi/passages.db`).
      5. **Decimal fractions excluded.** A real population count is always a bare integer in this codebase; `\d+` alone was splitting `10.0` into two separate digit matches, and the fractional `0` slipped past the `key=N` exclusion. Fixed `verify-g-kg-19-interval-convention.sh`'s false positive ("...(t=10.0) lands...").
      Each fix was independently re-verified: the `--prove-failure` battery stayed 6/6 after every change (proving the sharp-defect detection was never weakened), and three consecutive full fleet runs after all five fixes landed all read **SAFE=108 EXEMPT=0 FLAGGED=134 FAIL=0 of 242 checked**, byte-identically, before the check-registry additions below grew the population to 251.
      **A SIXTH, more serious defect was found and fixed in the sweep tool itself, not in a checked gate — this one was a real safety issue.** Running the sweep with a tight `--timeout-shell 8` left `scripts/crossref.sh` (a real production script) in a mutated, unrestored state (`elif False: # MUTATED-M3`) after the sweep's own runs, discovered via a routine post-run `git status`. Root cause: `prove-crossref-cli.sh` naturally takes **~20s** to complete its own mutation-and-restore cycle (measured directly, `time bash prove-crossref-cli.sh` → real 0m19.8s), and Python's `subprocess.run(timeout=N)` kills an overrun child via **SIGKILL**, which no `trap ... EXIT INT TERM` can catch — so the proof's own restore-on-exit trap never got to run. Reproduced deliberately to confirm the mechanism, then fixed: the runner now uses `subprocess.Popen` in its own process group, sends **SIGTERM** first on timeout, waits a **10s grace period** for the trap to complete its restore, and only escalates to SIGKILL — with an explicit "VERIFY IT BY HAND" note in its own output — if the process is still alive after that. Default timeouts were also raised (8s/8s/15s → 30s/20s/20s) so a real proof's natural runtime does not routinely trigger the timeout path at all. Verified directly: a standalone repro of the old code SIGKILLed and left the file mutated even with a 5s grace period; after the fix, a full real fleet run with the new (safe) defaults left `scripts/crossref.sh` `git status`-clean throughout, and `prove-crossref-cli.sh` itself completed normally within the new 30s budget.
      **Residual, honestly bounded, and not chased further.** After registering 9 new checks from this same phase (population 242→251), two more FAIL rows appeared and were manually verified as a SEVENTH, narrower false-positive class: prose describing an *exit code* using the word "gate" without an `=` sign (`"...an unmodified copy passes the gate: rc=0..."`, `"...the gate's 0, 1 and 2..."`) — both proofs (`prove-registry-derived-leak.sh`, `prove-producer-verifier-split.sh`) are genuinely healthy on inspection (10/10 and 5/5 arms passing respectively). This class was not patched: it is narrow, this codebase's dominant vocabulary ("gate", exit codes 0/1/2) makes it inherently ambiguous without deeper parsing, and the sweep's own three-valued design already treats FAIL as "a small, human-reviewable list to check," not an auto-trusted verdict — which is exactly what let this residual surface honestly rather than being silently absorbed.
      **Registered** as `vacuous-truth-fleet-sweep` in `check-registry-002.tsv`. `verify-check-registry-002.sh`: 196/196 after this task's own 8 rows landed alongside T470's (below).
- [x] T470 (was: 005/T054) **OPEN** — satisfy SC-014: **100%** of requirements map to at least one automated check, and every such check ships a data-driven paired demonstration (FR-020→unified FR-259, SC-014→unified SC-135). **This file is the first half of that criterion and does not complete it.** Every FR and SC in spec.md is now cited by at least one task above — but a task is not a check, and 22 of the 54 tasks here are not ticked. **Requirements with no automated check at all: FR-009, FR-010, FR-013, FR-014, FR-017 and SC-015.** Those six are the real coverage gap.
      **[DONE 2026-09-16] All six named requirements now have a real, paired-mutation-proven automated check. FR-014's own investigation set the pattern this task followed for the rest: a real mechanism already existed and was simply never registered — bookkeeping, not missing capability — for three of the six; genuine new coverage was built for the other three.**
      - **FR-014** (loopback auth): `internal/api/access.go`'s G-HTTP-9, already fully implemented and tested (`access_test.go`, three cases a/b/c) since Phase 6 of this session, but registered in **no** check-registry file. Confirmed live and correct — re-exercised for the first time this session against the T476 container rebuild (see T477).
      - **FR-009** (checksum-manifest missing/corrupted → refuse): already fully implemented (`scripts/extract-videos.sh`'s `require_manifest_or_refuse`, G-CLI-19) and already fully proven (`scripts/prove-manifest-precondition.sh`, 4 real cases including both missing- and corrupt-manifest arms plus a paired mutation), committed 2026-09-15 — never registered. Re-run: **8/8 pass**. Registered as `G-CLI-19` in `check-registry-008.tsv`.
      - **FR-010** (stable, non-positional, non-content-derived passage identity): the library-level guarantee already existed with real paired mutations in `submodules/passage/pkg/passage/pid_test.go` (`TestG1`/`TestG2` + their mutation arms, constructing an actual content-hash-derived and an actual position-derived `KeyStrategy` and showing both fail). The real gap: nothing in workshop's own test suite exercised WORKSHOP's OWN wiring end to end. Built `internal/passagestore/stable_identity_test.go`'s `TestFR010_StableIdentitySurvivesReorderAndCorrection` — mints synthetic segments through the real `Sync`/`Minter` path, re-syncs with segments reordered and with one corrected, asserts zero new mints and the same PID both times, then proves two throwaway mutant schemes (positional, content-hash) actually change under the same operations. Registered as `FR010-stable-identity` (go-test) in `check-registry-002.tsv`.
      - **FR-013** (private-derivable evidence stays inside the private module's own boundary): measured the two named umbrella-root shared-evidence conventions (`.test-evidence/<stamp>/`, `_tests/evidence/`) and confirmed no workshop script currently references either. Built `platform/gates/verify-evidence-location.sh` (G-EVID-1): a static source scan for references to those conventions, plus a filesystem scan of the two real shared directories for transcript/speaker-shaped content markers. `--prove-failure`: **6/6**. Registered as `G-EVID-1`/`G-EVID-1-proof`.
      - **FR-017** (chapter-id depth unbounded by construction, never asserted): confirmed correct by construction (`chapterid.go`'s grammar carries no depth cap), but nothing had tested past depth 3. Built `TestGCH22_DepthIsUnboundedByConstruction` (synthetic ids at depth 6/10/50/100, full derivation/sort/`ChildIDs` round trip, no special-casing) and `verify-chapter-depth-unbounded.sh` (G-CH-22). `--prove-failure` mutates the real `ChapterIDGrammar` constant in place (capped at depth 4, backup+trap+sha256-restore), confirms the mutation CAUGHT, restores byte-identically. **4/4**.
      - **SC-015** (0 hierarchy fields stored, measured across registry schema / derived database / every serialised artifact): built `verify-no-hierarchy-fields.sh` (G-CH-23), the real three-substrate measurement the requirement's own text names — DDL text + Go struct tags (12 files), the actual `.sqlite` files on disk via `PRAGMA table_info` (2 files), and every real `curriculum/*.jsonl`/`*.json` artifact's JSON keys (147 files) — all exact-name blocklist matching, verified non-substring via a dedicated negative control (`chapter_slug`, `scope_order`, `depth_gauge_reading` correctly NOT flagged). `--prove-failure`: **7/7**, including that negative control.
      **A genuine pre-existing defect was found and correctly NOT fixed, per investigate-before-fixing.** `verify-chapter-hierarchy.sh`'s G-CH-3/G-CH-5 checks scan a doubled path segment (`"$WORKSHOP_ROOT/workshop/platform/backend"`) that has never existed, so both have silently scanned nothing and reported false PASS since written. Fixing the path alone was confirmed to surface a SECOND, deeper defect (the heuristic cannot distinguish a persisted field from the FR-020-mandated derived HTTP-response DTO), so the path fix was **reverted** rather than shipped as an unreviewed side effect of unrelated work — `git status --short platform/gates/verify-chapter-hierarchy.sh` confirms it unmodified. SC-015 compliance does not depend on this pre-existing bug being fixed, since `verify-no-hierarchy-fields.sh` is the real, independent measurement. Recorded here as a follow-up, not resolved.
      **Full regression, independently re-verified**: `go build ./...`, `go vet ./...`, `go test -count=1 ./...` — all 24 backend packages clean, multiple full re-runs across this task and T469. `verify-check-registry-002.sh`: **196/196**, 0 missing. `verify-check-registry-001.sh` and `verify-limits-completeness.sh` unaffected by this task's own changes (see T477's write-up for a genuine, unrelated finding those two surfaced from the container rebuild).
...
- [x] T476 (was: 006/T050) [TDD] [SUBAGENT] [US5] Add the build identifier to `/api/health` (FR-026→unified FR-266). **Evidence needed**: nothing noticed the container was four hours older than the code, because no reading could be tied to a revision.
      **[DONE — already built in an earlier session commit (`e6f6ee9`, spec 008 Phase 6), re-verified and put into live production for the FIRST time this session, 2026-09-16.]** `platform/backend/cmd/workshop-server/build_identity.go` fully implements this: link-time `-X main.buildID=...`/`sourceCommit`/`sourceDirty`/`builtAt` (set by `scripts/build.sh`), falling back to `debug.ReadBuildInfo()`'s vcs.* stamp when unset, with a dirty tree always marked (never a bare commit hash while uncommitted changes are compiled in). Wired into `/api/health` at `main.go:2642`. Dedicated gate `platform/gates/verify-build-identity.sh` (B1 = the built artifact, B2 = the running deployment) is registered in `check-registry-006.tsv`.
      **What this task's remaining work actually was: the container had never been rebuilt with any of this session's changes, so B2 (the served claim) had never been exercised against a real deployment — every prior run of the gate reported B2 UNDETERMINED (unreachable) or, once reachable, showed the STALE build.** Re-verified this session:
      - Before rebuild: `bash scripts/status.sh` showed `build: "807f2c1-20260909T183130Z-dirty"` — the same stale, dirty build this whole session's `CLAUDE.md`/tasks.md notes have consistently recorded as never redeployed.
      - Rebuilt from a clean tree at HEAD `8209cca572fae844f437c64b341a5ad329d1bcf7` (T461's commit) via `bash scripts/build.sh` (clean run, Angular bundle + area exports all current), then `bash scripts/restart.sh` (full down/up per its own documented no-compose-restart discipline; named volumes explicitly preserved, `search index and transcripts intact` printed).
      - Post-rebuild, `curl http://127.0.0.1:8087/api/health`: `"build": "8209cca-20260916T014703Z", "source_commit": "8209cca572fae844f437c64b341a5ad329d1bcf7", "source_dirty": false` — matches `git rev-parse HEAD` in `workshop` exactly.
      - `WORKSHOP_BASE_URL=http://127.0.0.1:8087 bash platform/gates/verify-build-identity.sh`: **2 pass, 0 finding(s), 0 undetermined** — both B1 (built) and B2 (served) now PASS and agree, the first time this gate has ever graded a live, matching, non-dirty deployment this session.
      - Incidental positive finding from this same rebuild: G-HTTP-9 (loopback-only access control, built earlier this session but never live) correctly refused a non-loopback probe on its very first live exercise — `curl http://192.168.1.115:8087/api/health` (LAN address) returned `401 {"error":{"code":"unauthorized",...}}`, while `curl http://127.0.0.1:8087/...` succeeded. The server still binds `0.0.0.0:8087` (confirmed via `podman inspect ... Config.Cmd`); the access-control layer is what's actually gating the LAN address, exactly as designed.
- [x] T477 (was: 006/T051) [SUBAGENT] [US5] Commission a **second** independent adversarial review against the rebuilt container (FR-029→unified FR-269, SC-014→unified SC-140). The first found four false headline claims; a review that confirms is worth little.
      **[DONE 2026-09-16]** Commissioned a fresh, independent subagent with an explicit falsification mandate (not a confirmation mandate) against the just-rebuilt container (`8209cca-20260916T014703Z`, T476's rebuild). It genuinely tried to break 11 distinct claims via live loopback HTTP requests, `podman exec`/`podman inspect`, source-code tracing and real test execution — nothing modified, read-only throughout.
      **Result: this round found no new false headline claim, and the specific defect class the prior review is named for (a kind advertised in `indexed_kinds` with zero retrievable results — the "diagram" shape) is demonstrably fixed**: `?kinds=diagram` now returns HTTP 503 `kind_not_indexed` with an honest, specific message, rather than a silent `no_match`. The `/api/ask` answer endpoint (currently `-answer-provider none`) correctly reports HTTP 503 `no_provider` rather than hanging or fabricating. G-HTTP-9 loopback access control was independently verified on a route not previously exercised live (`/api/search`), by three methods: live 401 from the LAN address, live 200 from loopback, and by tracing `main.go`'s actual `http.Server.Handler` wiring (all 34 `mux.Handle*` registrations sit on one mux, wrapped once by `LoopbackOnly` before becoming the server's real handler) rather than trusting the doc comment's claim.
      **Two real, non-blocking findings surfaced and independently re-confirmed by me before accepting them**: (1) `workshop/README.md` line 659, "The corpus is one chapter", is stale — the live server now truthfully reports and serves **4** chapters (`01`, `02`, `02.01`, `02.02`), confirmed via `/api/health`, `/api/chapters` and the container filesystem; (2) the "41 of 41 areas published" figure, while literally accurate for what `/api/areas` lists, coexists with one additional held-back area (`held_back: {count:1, reason: stale_publication_review}`) that correctly 404s rather than silently serving — the system is more honest than the summary phrasing, but the phrasing itself invites misreading. One item (SC-010 fabrication re-measurement) is correctly reported UNDETERMINED rather than confirmed/refuted, since the disabled answer provider makes that scenario structurally unexercisable today — an honest "cannot be tested right now," not a bluffed pass.
      **Independently re-verified by me, not merely accepted from the subagent's report**: re-ran the exact `curl` calls for the health identity, the diagram-decoy search, the `/api/ask` unavailable response, the non-loopback 401, and the README line — every one matched the subagent's report byte-for-byte or line-for-line.
      **Honest boundary**: this review is scoped to the claims listed; it is not an exhaustive security audit of the whole platform, and finding "nothing new broke this round" is itself the meaningful, non-vacuous outcome the task calls for — a review that always finds something would be as suspect as one that never does.
      **A separate, genuine finding surfaced independently while re-running this phase's own gates against the now-live container (not part of the commissioned adversarial review itself, but the same root cause: nothing had exercised the real container this session before now).** `bash platform/gates/verify-check-registry-001.sh`'s `status-exit-contract` R4 probe FAILs: `verify-status-exit-contract.sh --skip-live --quiet`'s A1 arm fakes an absent compose project to force a STOPPED verdict, but `status.sh`'s own G-CLI-12 safety check (pre-existing, commit `a97f782`, well before this session) correctly detects that the real target port (127.0.0.1:8087) is occupied by a process it cannot attribute to the FAKE project name, and honestly reports UNDETERMINED instead of a false STOPPED — which is G-CLI-12 working exactly as documented, not a code regression. The actual defect is in the PROBE's own isolation design: A1 implicitly assumes the real production HTTP port is free during its test, which was true every time this gate ran earlier in this session (nothing was ever live), and is no longer true now that T476 finally started a real, legitimately-running container. **Not fixed** — out of scope for Phase 15 (a different gate, from spec 001, with a test-isolation design question rather than a simple bug), confirmed pre-existing via `git log`, and recorded here rather than silently worked around by stopping the live server to make the gate read green.
...
**43 tasks — 43 complete, 0 open.** Sources: 004, 007, plus 8 tasks new in the 2026-09-15
...
- [x] T484 (was: 007/T030) [P] [US1] Add decommission gate for workshop: the umbrella-hosted `workshop/` copy remains the operative one and MUST NOT be removed/decommissioned until the standalone workshop clone independently passes `install` and the full test suite on its own (FR-052→unified FR-328)
      **[DONE 2026-09-16]** `scripts/verify-decommission-precondition.sh <module> [--clone path]` (umbrella root; parameterized, one script for both T484/T487). Locates the named standalone clone, runs its own `scripts/build.sh` then `go test ./...` independently, and REFUSES to authorize decommissioning unless both genuinely pass — three-valued (rc 2 when no clone is found). `--prove-failure`: 6/6 mutations. Registered as `decommission-precondition` in `scripts/check-registry.tsv`.
      **Real, honest current state (not a manufactured pass): NOT SAFE TO DECOMMISSION, rc=1.** `bash scripts/verify-decommission-precondition.sh workshop` against the real standalone clone (a separate checkout outside this umbrella, per T482's own "clone workshop repo outside umbrella vasic"): `scripts/build.sh` succeeds; `go test ./...` genuinely FAILS (`TestGateKG1_ChangedEndpointsCiteTheDeltaContract`, a standalone-checkout limitation that test names explicitly — it needs `specs/*/contracts/http-api-delta.md`, absent outside the superproject). The umbrella-hosted copy correctly remains the operative one. Independently re-verified this session.
...
- [x] T487 (was: 007/T031) [P] [US2] Add decommission gate for ai_interviewing: the umbrella-hosted `ai_interviewing/` copy remains the operative one and MUST NOT be removed/decommissioned until the standalone ai_interviewing clone independently passes `install` and the full test suite on its own (FR-052→unified FR-328)
      **[DONE 2026-09-16]** Same `scripts/verify-decommission-precondition.sh` as T484, parameterized for `ai_interviewing`.
      **Real, honest current state: COULD NOT DETERMINE, rc=2.** Against the real standalone clone (a separate checkout outside this umbrella, per T485's own "clone ai_interviewing repo outside umbrella vasic"): `platform/scripts/build.sh` succeeds (including the Angular frontend) and `go test ./...` genuinely PASSES — but the gate correctly refuses SAFE rather than silently calling itself "the full suite", because `platform/qa/e2e`'s Playwright suite is disclosed as not executed (needs browsers + a live server) — a genuine, named gap, not glossed over. Real finding surfaced along the way: `ai_interviewing` has no `scripts/bootstrap-standalone.sh` (workshop has one); its `verify-standalone-clone.sh` exists but is the narrower static T479 check. The umbrella-hosted copy correctly remains the operative one.
...
- [x] T488 (was: 007/T005) [TDD] [US3] Setup shared DB schema and migrations for auth (users milosvasic/rami, roles admin/user)
      **[RE-VERIFIED 2026-09-16, PREVIOUSLY TICKED WITH NO EVIDENCE TRAIL.]** This box, and T489 below, had been `[x]` since before this pass with zero evidence text — an outlier against this document's own convention of citing exact files/tests for every closure, and the trigger for this whole phase's investigation. **Both are now confirmed genuinely built, independently, for real, in both modules — not a bare assertion.** Workshop: `platform/backend/pkg/authstore/authstore.go` (schema + migrations, auto-seeded on `Open()`), tests `TestWorkshopSeededUsers` etc. all real PASS. ai_interviewing: `internal/store/auth.go` (`authSchema` DDL + `seedAuth()`), `TestAuthSchemaSeededUsers` real PASS. See T502-T512/T577-T579 below for the hardening built on top this session.
- [x] T489 (was: 007/T006) [TDD] [US3] Implement login form and RBAC on both modules (workshop: full access; ai_interviewing: rami restricted on employer_data/github_analysis)
      ...
      **[RE-VERIFIED 2026-09-16.]** Workshop: `TestWorkshopBothRolesFullAccess`/`TestRamiFullAccessOnWorkshop` real PASS. ai_interviewing: `TestRamiCannotReadEmployerOrGithubTracks` (direct `GET /api/modules/{id}` on a restricted module returns real 403, admin gets real 200) and `TestRamiCannotSearchEmployerContent` real PASS — RBAC is role-based (`auth_permission(module,resource,action,role)`), NOT a `username=="rami"` branch; the restriction is the documented ABSENCE of rows for role `user`. **Precise nuance, measured not guessed**: adding a new resource/action rule for an EXISTING role is zero-code (pure data); adding a genuinely new ROLE is schema-capped (`CHECK(role IN ('admin','user'))` in both modules) — see T579 for the gate that reports this honestly rather than overclaiming.
...
- [x] T494 (was: 007/T028) [REVIEW] [US5] Add a mandatory content-boundary review gate to the reusable-code extraction pipeline: run the `scripts/verify-content-boundary.sh`-class check against each candidate extraction from workshop/ai_interviewing and block creation/push of the new public vasic-digital repository until it confirms zero private data (FR-051→unified FR-327)
      **[DONE 2026-09-16]** `scripts/verify-extraction-content-boundary.sh --candidate <dir> [--private workshop|ai_interviewing|<path>]...` — wraps the EXISTING `scripts/verify-content-boundary.sh` class of check (never reimplements detection) with zero-tolerance scoping: always passes an empty allow-list, ignoring both the umbrella's own `.content-boundary-allow` and any candidate's self-declared one — an extraction about to go public gets no exemptions the already-published umbrella carries. `--prove-failure`: 5/5. Registered as `extraction-content-boundary`.
      **Real run against a real candidate: BLOCKED — LEAK, rc=1**, `submodules/passage` vs. private `workshop`: 1707 matches, almost entirely governance-prose/authorial-style echoes (the same class-C phenomenon this repository's own CLAUDE.md documents extensively), not obviously verbatim disclosures — but the gate correctly does NOT auto-clear them. **Honest implication, stated plainly**: as built, this zero-tolerance gate will block essentially any real extraction candidate sharing this project's writing style with its private source, and requires human judgment on each surfaced match. This matches the task's literal ask ("block... until it confirms zero private data") — the gate is a mandatory human-review trigger, not a silent auto-pass, which is the correct and intended shape for this precondition.
- [x] T495 (was: 007/T029) [P] [US5] Implement additive-then-cutover extraction pattern: keep each existing in-tree reusable code path functional in workshop/ai_interviewing until the corresponding consumer is confirmed migrated to, and re-tested against, the extracted public submodule (FR-053→unified FR-329)
      **[DONE 2026-09-16]** `scripts/verify-additive-cutover.sh [--consumer <dir>] [--pair <import>:<dir>]...` — a static R/X/T/C check (Require declared / repLace present / Target resolves / Consumed by a real import) over (import-path, target-dir) pairs against a consumer `go.mod`, defaulting to workshop/platform/backend × {passage, verdict, curriculum-kit}. `--prove-failure`: 6/6 mutations (omitted replace, non-resolving target, module-name mismatch, resolving-but-never-imported, absent go.mod). Registered as `additive-cutover`.
      **Real run: CUT OVER, rc=0** — all three of passage/verdict/curriculum-kit are genuinely required, replaced, resolving, and imported at real call sites (`internal/answering/wire.go`, `internal/passagestore/passagestore.go`, `internal/api/assessment_disclosure.go`).
      **Honest process finding, not glossed over**: `workshop/platform/backend/go.mod` was only touched in 2 commits in that repo's own history, and in BOTH, the `replace` directive landed in the SAME commit that first introduced the consuming code — no case has yet required migrating AWAY from a pre-existing in-tree implementation. The additive-then-cutover discipline this task exists to guard has therefore not actually been exercised or tested by history yet (it was never violated, but also never demonstrated) — this gate is exactly the mechanism that will catch it the next time an extraction DOES need to replace real in-tree code.
...
- [x] T502 (was: 007/T017) [TDD] [REVIEW] [US3] Implement salted password hashing (Argon2id or bcrypt) for the shared user store on both modules; verify by test that no plaintext or reversibly-encrypted password value exists in any table, log, or fixture (FR-042→unified FR-305)
      **[DONE 2026-09-16]** Both modules already used real bcrypt-12 (which the task's own text names as satisfying, "Argon2id **or** bcrypt") — confirmed by reading the actual call sites, not inferred from a name. The genuine gap was the missing negative-assertion test, now built: workshop `TestT502_NoPlaintextOrReversiblePasswordAnywhereOnDisk` (scans raw `.db`/`-wal`/`-shm` bytes), ai_interviewing `TestSeedPasswordHashesAreBcryptNotPlaintext` (queries the real DB, asserts `$2[abxy]$` shape, not-equal-to-plaintext). Both real PASS.
- [x] T503 (was: 007/T018) [P] [REVIEW] [US3] Add a pre-deployment boot guard that refuses to start a production-flagged environment while the FR-009 example password hashes (for the two seed accounts — **the two literal password values are REDACTED in this merged copy; they remain readable, unchanged, at `specs/007-decouple-modules-auth/tasks.md` 007/T018 (unified T503)**) are still active, and document in each module's README that these already-committed values are compromised and MUST NOT be reused as real secrets (FR-043→unified FR-306)
      **[DONE 2026-09-16]** Workshop: `-env`/`WORKSHOP_ENV` flag, `checkProductionBootGuard` called first in `run()` before anything is bound. ai_interviewing: `internal/bootguard`, `AI_INTERVIEWING_ENV`. Both: `production` + unrotated seed hash(es) → refused with a message naming which account(s); `production` + rotated → boots; dev mode → boots regardless. Real tests both modules (workshop: `TestT503_RefusesWithUnrotatedSeedHashes`/`AllowsAfterRotatingBothSeedHashes`/`PartialRotationStillRefuses`/`DevModeBootsWithDefaultHashes`/`ProductionRunRefusesBeforeBindingAnything`; ai_interviewing: `TestCheck_ProductionRefusesUnrotatedSeedHashes`/`DevModeBootsNormally`/`ProductionBootsOnceSeedHashesAreRotated`/`ProductionRefusesPartialRotation`) all real PASS. Both READMEs document the seed values as compromised examples.
- [x] T504 (was: 007/T019) [TDD] [REVIEW] [US3] Add automated log/error-output scanning (unit + integration test) asserting that session tokens, session identifiers, and passwords never appear in application logs, error responses, crash reports, or URL/query strings on either module (FR-044→unified FR-307)
      **[DONE 2026-09-16]** Both modules: exercise login (success + failure) and error responses, capture real stdout/stderr, assert no plaintext password or token appears anywhere in log output or HTTP error bodies. Workshop: `TestT504_AuthLogsAndResponsesNeverLeakSecrets`. ai_interviewing: `TestLoginNeverLeaksSecretsInLogsOrErrorBodies` (0 bytes logged; response bodies checked directly too). Both real PASS.
- [x] T505 (was: 007/T020) [TDD] [REVIEW] [US3] Implement session-ID regeneration on successful login on both modules; add a regression test proving the pre-authentication session id is discarded and cannot become the authenticated session id (session-fixation protection) (FR-045→unified FR-308)
      **[DONE 2026-09-16]** Fixation-safe behavior already existed in both modules by construction (login always mints a fresh `crypto/rand` token; no code path takes a caller-supplied id). The genuine gap was the missing dedicated regression test, now built: an attacker-fixed pre-auth cookie/token is sent through a real login; asserts the post-login token differs, the pre-auth value stays invalid post-login, and the real token authenticates. Workshop: `TestT505_SessionFixation_LoginNeverReusesAPreAuthToken`. ai_interviewing: `TestLoginRegeneratesSession_FixationRegression`. Both real PASS.
- [x] T506 (was: 007/T021) [TDD] [REVIEW] [US3] Implement audience-scoped (module-restricted) tokens; add a contract test proving a token minted by workshop is rejected by ai_interviewing and vice versa, even though both modules read the shared user store (FR-046→unified FR-309)
      **[DONE 2026-09-16]** Neither module had ANY audience/module binding on its session tokens before this session — a token from either module would authenticate against the other (confirmed absent by direct code reading before building anything). Built a new shared, standalone, independently-tested Go module `submodules/authkit` (`github.com/vasic-digital/authkit`, `pkg/session`: real HMAC-SHA256 audience-scoped tokens, `Mint(sessionID, audience)`/`Validate(token, expectedAudience)`), wired ADDITIVELY into both modules' login/switch flows alongside their existing (unmodified, still load-bearing) token systems — env `AUTH_SHARED_SECRET`, identical name and convention in both modules by design, so a token minted by one is validated by the SAME shared code the other imports, not a reimplementation that could drift. Real contract proof, both directions, both modules: workshop `TestT506_LoginMintsAudienceScopedAuthKitToken` mints with audience `"workshop"`, validates correctly for `"workshop"`, and is REJECTED (`session.ErrAudienceMismatch`) for `"ai_interviewing"` via authkit's own `Validate`; ai_interviewing's `TestAuthkitMintForUser_RejectedUnderForeignAudience` proves the same in its own direction. Since both modules import the identical shared `Validate` function with the identical secret, this is a complete, non-redundant proof of the cross-module property — a separate literal cross-repo test would exercise the same code twice. `authkit` itself independently verified first: go build/vet/test clean, real §1.1 mutation proofs for every property (salting, audience rejection, fixation-safety, fail-closed, rate limiting, generic failure messages).
- [x] T507 (was: 007/T022) [TDD] [REVIEW] [US3] Implement fail-closed behavior on both modules when the auth provider/user database is unreachable; add an integration test that simulates database unavailability and asserts every protected route denies access (never grants it) during the outage (FR-047→unified FR-310)
      **[DONE 2026-09-16]** Both modules were already fail-closed BY CONSTRUCTION (any DB error is treated identically to an invalid credential — confirmed by reading the code, not assumed). The genuine gap was proving it under a REAL simulated outage, end to end through the HTTP layer, not just at the function level — now built: close the actual `*sql.DB` connection the live test server is using, mid-test, and assert every protected route denies, a fresh login also fails, and recovery works once a fresh connection is available. Workshop: `TestT507_HTTPLayerDeniesEveryProtectedRouteDuringAuthDBOutage`. ai_interviewing: `TestProtectedRoutesDenyDuringDBOutage`. Both real PASS.
- [x] T508 (was: 007/T023) [TDD] [P] [US3] Implement per-account and per-source login rate limiting/throttling on both modules; add a test that exceeds the threshold and asserts subsequent attempts are throttled/blocked (FR-048→unified FR-311)
      **[DONE 2026-09-16]** No rate-limiting code existed in either module before this session. Wired `authkit`'s real sliding-window `pkg/ratelimit` into both modules' login handlers, keyed by username AND remote address (both axes apply, checked before the credential check so a blocked caller never reaches bcrypt). Real tests, both modules: exceed the threshold, assert 429 on the next attempt, assert recovery after the window. Workshop: `TestT508_LoginRateLimiting_BlocksThenRecovers`, `TestT508_LoginRateLimiting_PerSourceAppliesAcrossDifferentAccounts`. ai_interviewing: `TestLoginRateLimiting` (live HTTP: 3× 401 then 429 on the 4th). Both real PASS, including live-container confirmation (see T580/T581 for ai_interviewing: a real rate-limit boundary measured against the live container, not just a unit test).
- [x] T509 (was: 007/T024) [TDD] [P] [US3] Implement generic ("invalid username or password") authentication failure messages on both modules; add a test asserting byte-identical response content/shape for an unknown username vs. a known username with a wrong password (username-enumeration prevention) (FR-049→unified FR-312)
      **[DONE — ALREADY SATISFIED, RE-VERIFIED 2026-09-16]** Both modules already collapsed unknown-user and wrong-password to one generic response (confirmed by reading the code and by re-running the existing real tests). Workshop: `authLoginHandler` returns identical `{"error":"invalid_credentials","message":"Invalid username or password"}` for both; `TestAuthedServerBadCredentials` real PASS. ai_interviewing: `LoginSession` collapses to one `ErrInvalidCredentials`; `TestAuthedServerBadCredentials`/`TestLoginRejectsBadCredentials` real PASS.
- [x] T510 (was: 007/T025) [TDD] [P] [US3] Exempt health-check/readiness endpoints from authentication on both modules; add a test asserting these endpoints return HTTP 200 with no content or user data of any kind (FR-050→unified FR-313)
      **[DONE — ALREADY SATISFIED, RE-VERIFIED 2026-09-16]** Workshop: `wrapWithAuth`'s `authMux.Handle("GET /api/health", content)` bypasses `authhttp.Middleware`; `TestAuthedServerHealthStaysPublic` real PASS (after this session's own test-fixture fix — see T506's integration note on `loopbackRequest`, required because the umbrella copy's `routes(cfg)` also applies G-HTTP-9's loopback check, a property these ported tests were not originally written against). ai_interviewing: `/health` registered outside the auth-required route group; `TestAuthedServerHealthStaysPublic` real PASS.
- [x] T511 (was: 007/T026) [TDD] [REVIEW] [US3] Implement default-deny access for any user record with no assigned role/permission on both modules; add a test creating a roleless user and asserting zero access is granted — never admin-equivalent or inherited access (FR-054→unified FR-314)
      **[DONE 2026-09-16]** Both modules' schemas make a roleless row structurally IMPOSSIBLE (`role TEXT NOT NULL CHECK(role IN ('admin','user'))`) — a stronger property than "assert zero access on a roleless row", since no such row can ever exist to be checked. Real tests demonstrate the INSERT itself is rejected (NULL, invalid, and empty-string role all fail; a valid role still inserts, as a control). Workshop: `TestT511_RolelessUserRowIsUnrepresentable` (4 subtests). ai_interviewing: `TestT511_RolelessUserRowIsUnrepresentable` (4 subtests). Both real PASS.
- [x] T512 (was: 007/T027) [TDD] [REVIEW] [US3] Implement cutover invalidation of any session/cookie issued before secure session handling (FR-017→unified FR-304) was enforced; add a migration test proving a pre-rollout session cookie is rejected post-cutover on both modules (FR-055→unified FR-315)
      **[DONE 2026-09-16]** No mass/cutover session-invalidation mechanism existed in either module before this session. Built on `authkit`'s `session.Manager.InvalidateAllBefore(timestamp)`: every token issued before a given timestamp is rejected from that call onward, regardless of individual session state — the credential-rotation / security-incident primitive this task needs. Real migration tests, both modules: mint a token, set the cutover AFTER that mint time, assert rejected; mint a token AFTER the cutover, assert it still validates. Workshop: `TestT512_CutoverInvalidatesOlderTokensNotNewerOnes`, `TestT512_CutoverIsANoOpErrorWhenSecretUnset`. ai_interviewing: `TestAuthkitInvalidateAllBefore_Cutover`. Both real PASS. Two genuine timing bugs were found and fixed during TDD, root-caused before fixing: a too-tight rate-limit test window pruned hits before its own budget was exhausted (real bcrypt-12 compares cost ~0.6-1s), and a mint immediately after capturing a cutover timestamp could floor into the same second as the cutover under `Mint`'s whole-second issued-at truncation.
...
- [x] T574 (new — 2026-09-15 remediation pass, no source-spec origin) [TDD] [US3] Build the **cutover census**: enumerate every anonymous per-browser learner-progress record that exists at the moment authentication is switched on, and write the census — record id, created-at, last-touched, browser-scope key — to a durable artefact under `workshop/evidence/` **before any record is touched**. Gate **G-AUTH-1**: the census artefact exists and is written before the cutover migration runs; a cutover that runs with no census present FAILS. **Paired mutation**, data-driven and with no edit to the gate: seed a fixture store carrying anonymous records, delete the census artefact, re-run — the gate MUST go red. Three-valued: **rc 2** where the progress store cannot be read, and an rc 2 is never a pass. (unified FR-321, unified SC-161)
      **[DONE 2026-09-16]** `internal/api/progress_census.go`'s `ProgressStore.AnonymousCensus()` (real method on the real `ProgressStore`, reusing its existing `load()`), excluding any key already carrying the new `authhttp.ProgressKeyPrefix` ("auth-user:") so an already-migrated account is never double-counted. `pkg/migration/census.go` builds/writes/reads the durable JSON artefact and exposes `PrepareCutover(path)`, which fails closed with `ErrNoCutoverCensus` when the artefact is missing. New CLI `cmd/migrate-progress-census` (`-store -out -check`). `platform/gates/verify-g-auth-1-workshop.sh`, real PASS. `--prove-failure`: control (census present) rc=0; mutation (census deleted, same CLI re-checked) rc=1 naming the missing path — never silently treated as "nothing to migrate".
      **Zero anonymous records is a determined, valid census (rc 0), never conflated with an unreadable store (rc 2).**
- [x] T575 (new — 2026-09-15 remediation pass, no source-spec origin) [TDD] [US3] Present the learner an **explicit choice** at first authenticated login — migrate the anonymous record into the new account, or discard it with notice — and log the chosen outcome, with its timestamp and the acting account, against the record id. **No silent merge and no silent discard**: unified FR-321 forbids both by name, the first because it attributes one person's activity to another on a shared browser, the second because it destroys a record FR-118 promised to preserve. Gate **G-AUTH-2**: for a seeded anonymous record, an authenticated login that neither prompts nor records an outcome MUST fail. **Paired mutation**: replace the prompt with an automatic merge in a throwaway copy — the gate MUST go red on the same fixture. (unified FR-321)
      **[DONE 2026-09-16]** `cmd/workshop-server/migration_wiring.go`: `authLoginHandler`/`authSwitchHandler` add an optional `"pending_migration_choice": {record_id, created_at, last_touched, migrate_href}` to the login response when the request's `X-Session` matches an unresolved T574 census record. New endpoint `POST /api/auth/migrate-progress` (`{"record_id","choice":"migrate"|"discard"}`, auth required) performs the real move (`ProgressStore.MigrateSession`, newer-timestamp-wins merge) or real removal (`ProgressStore.DeleteSession`) and appends to `pkg/migration/outcome.go`'s closed-vocabulary `OutcomeStore` — refuses a second write for the same record, so a decision cannot be silently revised either. `platform/gates/verify-g-auth-2-workshop.sh`, real PASS (8 tests). `--prove-failure`: a throwaway whole-file swap to a silent auto-merge (no prompt, no logged outcome) correctly turns the gate red.
      **Honest boundary, not glossed over**: no live-container/over-the-wire E2E for this flow — every test is in-process (`httptest` against the real `wrapWithAuth` handler) or unit-level. `platform/frontend` (Angular) was NOT touched — nothing in the UI reads `pending_migration_choice` or calls the new endpoint yet; a real, unaddressed follow-up. `pkg/learning.SessionStore` (lesson/assessment progress, also anonymously keyed by `X-Session`) is a SECOND anonymous store this migration does not cover — flagged, not silently assumed included.
- [x] T576 (new — 2026-09-15 remediation pass, no source-spec origin) [TDD] [REVIEW] [US3] Assert **100%** coverage exactly as unified SC-161 states it: every record in T574's census reaches one of the two stated outcomes, and the count of records handled silently is **0**. Gate **G-AUTH-3** reconciles the census against T575's outcome log and reports the residue **by record id, never as a percentage alone** — a percentage hides which record was lost. **Paired mutation**: drop one row from the outcome log — the gate MUST go red **naming that record id**, not merely reporting a lower figure. Three-valued: **rc 2** where either the census or the outcome log is unreadable. This is the evidence migration gate **M6** requires, and M6 is the only irreversible step in the migration. (unified FR-321, unified SC-161)
      **[DONE 2026-09-16]** `pkg/migration/reconcile.go`'s `Reconcile(census, outcomes)` returns `MissingRecordIDs []string`, never a percentage. New CLI `cmd/reconcile-migration-census`. `platform/gates/verify-g-auth-3-workshop.sh`, real PASS (7 tests) — proven BOTH as a pure function over hand-built fixtures AND over the real artefacts a real HTTP login+migrate flow produced. `--prove-failure`: dropped `browser-b`'s row from a real, on-disk outcome log; the reconciler correctly exited 1 and printed `missing_record_id=browser-b` by name, never a bare lower count.
      **Phase 16 is now 43/43 complete.**
- [x] T577 (new — 2026-09-15 remediation pass, no source-spec origin) [TDD] [US3] **Negative-assertion** gate **G-AUTH-4**: no self-registration route is reachable from the login page on either module. Enumerate every route the login page links to or posts to, assert that none creates an account, and assert over HTTP against both running modules that the account-creation path is not served. **Paired mutation**: add a registration route to a throwaway copy of the route table — the gate MUST go red. Three-valued: **rc 2** where a module is not running, because an unreachable service is never a pass. Pairs with T488/T489. (unified FR-316)
      **[DONE 2026-09-16]** Both modules: enumerate every registered route, assert none creates an account, plus a live probe of guessed registration-shaped paths asserting 404/405. Workshop: `TestT577_NoSelfRegistrationRouteReachable` (10 guessed routes + static route-table check). ai_interviewing: `TestNoSelfRegistrationRoute` (static `gin.Engine.Routes()` scan + live probe of 10 guessed paths × 2 verbs). Both real PASS, both confirmed zero registration-shaped routes exist.
- [x] T578 (new — 2026-09-15 remediation pass, no source-spec origin) [TDD] [REVIEW] [US3] Confirm the **seed-account bootstrap is operator-run and non-network-exposed** — a CLI step or a migration invoked by an operator, never an HTTP endpoint. Gate **G-AUTH-5** asserts the seed path exists as an executable script or migration, and that no HTTP handler in either module creates a seed account. **Paired mutation**: expose the seed routine behind a route in a throwaway copy — the gate MUST go red. (unified FR-316)
      **[DONE — ALREADY SATISFIED, RE-VERIFIED 2026-09-16]** Both modules seed the two mandated accounts automatically and idempotently inside their own `Open()`/migration path (workshop: `authstore.Open`; ai_interviewing: `seedAuth()`), never through an HTTP handler — confirmed by reading every registered route in both modules (already enumerated for T577) and finding no account-creation endpoint of any kind.
- [x] T579 (new — 2026-09-15 remediation pass, no source-spec origin) [TDD] [REVIEW] [US3] Prove the permission model is a **genuine `(user, module, resource, action)` rule set** rather than a two-user hard-coding: add a third tuple — a third user, or a third module/resource/action combination — **with zero code changes, data only**, and assert it takes effect. Gate **G-RBAC-1**: the added tuple grants exactly what it names and nothing more, and the two seed users' existing access is unchanged. **Paired mutation**: revert the rule set to a hardcoded two-user branch in a throwaway copy — the gate MUST go red, because the third tuple can then no longer be added as data. **This is the extensibility property unified FR-303 actually requires; see the NOTE on T489, whose literal two-user scope is narrower.** (unified FR-303)
      **[DONE 2026-09-16, both modules.]** ai_interviewing: `G-RBAC-1` real PASS — a third `(ai_interviewing, interview_transcripts, write, user)` tuple takes effect as pure data with zero code changes, both seed users' prior access unchanged; role dimension honestly reported schema-capped. Workshop: `G-RBAC-1` (`platform/gates/verify-g-rbac-1-workshop.sh`, `pkg/authstore/g_rbac_1_test.go`) real PASS, measured independently rather than assumed to match — **resource** dimension is data-driven (no CHECK), **action** dimension is data-driven but ALSO schema-capped (`CHECK(action IN ('read','write','delete'))`, a genuine difference from ai_interviewing found and reported honestly), **role** dimension schema-capped identically to ai_interviewing's own finding. `--prove-failure` both modules: reverted to a hardcoded two-user/role branch in a throwaway copy, confirmed the third tuple has no effect under the mutant while the real engine reacts to it. Neither gate claims full T579 compliance — both report their schema-capped dimension(s) explicitly rather than overclaiming.
- [x] T580 (new — 2026-09-15 remediation pass, no source-spec origin) [TDD] [US6] Boot both standalone modules as **live running containerised instances** through the shared `submodules/containers` module's **rootless** runtime — never a hand-rolled `podman`/`docker` literal, which §11.4.76 forbids as a parallel runtime implementation inside a consuming project — and assert each answers its health check. Gate **G-LIVE-1** records the runtime that the module's own `runtime.AutoDetect` resolved, the container identifier, and the health-check response body. **Paired mutation**: point the health check at a stopped container — the gate MUST go red rather than reporting an absent service as healthy. Three-valued: **rc 2** where no rootless runtime is available on the host, which is never a pass. (unified FR-293, unified SC-158)
      **[DONE 2026-09-16, both modules.]** ai_interviewing: `G-LIVE-1` real PASS — booted through `digital.vasic.containers/pkg/runtime.AutoDetect` (podman 5.7.0, no hand-rolled runtime literal) + `pkg/compose.NewDefaultOrchestrator`, container `ai-interviewing-glive1` genuinely running, real `GET /api/health` → HTTP 200 `{"ok":true,...}`. Workshop: `G-LIVE-1` (`platform/gates/verify-g-live-1-workshop.sh`) real PASS — REUSED workshop's own existing, daily-driven container control plane (`scripts/build.sh`/`start.sh`/`status.sh`/`stop.sh`, itself built on `digital.vasic.containers` via the separate `platform/orchestration/cmd/workshop-boot` module — confirmed zero hand-rolled `podman`/`docker` shell-outs by grep, every hit a comment) rather than building a second, parallel boot mechanism — the correct reuse call given one already existed. Rebuilt and restarted to serve this session's auth changes; live container confirmed running build `3e8526ac5ab...` (this phase's own workshop commit), `GET /api/health` → HTTP 200. `--prove-failure` both modules: a real stop (podman `runtime.Stop`/`scripts/stop.sh`) makes the health check correctly report unhealthy/connection-refused, not an absent-service false pass; restart restores it.
- [x] T581 (new — 2026-09-15 remediation pass, no source-spec origin) [TDD] [REVIEW] [US6] Run each standalone module's full install-and-test path **against the live containerised instance T580 booted**, not against a source checkout, and record every figure with its population stated as `served`. Gate **G-LIVE-2** fails if any figure recorded for this task carries population `source` or `in_process` — a measurement of source is never evidence about the served product. **Paired mutation**: relabel one served figure as `in_process` — the gate MUST go red. This is the live-instance evidence unified FR-293 and SC-158 require, and it is a precondition of the decommission gate T484/T487 enforce for M4. (unified FR-293, unified SC-158)
      **[DONE 2026-09-16, both modules.]** ai_interviewing: `G-LIVE-2` real PASS — every recorded figure `population:served` against the live G-LIVE-1 container: health, login (seed admin + seed user), wrong-password (401), protected route with valid/invalid token, real measured rate-limit boundary (3×401 then 429, this module's tuned 3/20s limit). Workshop: `G-LIVE-2` (`platform/gates/verify-g-live-2-workshop.sh`) real PASS — same shape against the live workshop container: health, login (both seed accounts, already-documented example credentials), `/api/auth/me` valid/invalid token, wrong-password (401 generic), and a real measured rate-limit boundary against this module's default limit (5 attempts 401, 6th → 429, measured live rather than assumed from source since workshop's compose file sets no override, unlike ai_interviewing's tuned test limit). `--prove-failure` both modules: a real served figure relabeled `in_process` in a mutated copy is correctly rejected by the integrity check.
...
**62 tasks — 54 complete, 8 open.** Sources: 001, 002, 003, 004, 006, plus 1 task new in the
2026-09-15 remediation pass with no source-spec origin. **The 8 open tasks are ALL genuinely
blocked and correctly left open, none silently skipped:** T521/T527/T530/T553/T554/T573 are
operator-only (redaction-review sign-off, a live push, a human final review, authored-schema
sign-off) with no agent action available; T540 and T543 are real work this session's parallel
tracks did not reach (T540 blocked on predecessors T042/T022/T084 not touched this session; T543
is genuinely unworked — see its own line) and are recorded here rather than guessed at.
...
- [x] T522 (was: 001/T131) [P] [REVIEW] Route this feature's automated-check evidence that could contain transcript excerpts, speaker attribution, or other content derived from the private `workshop` submodule to a location inside that submodule's own boundary (001/T099 (unified T521)'s `workshop/evidence/`), and reserve the public monorepo's shared `_tests/evidence/` convention for evidence provably free of such content — a bare pass/fail count with no quoted text. Extend the content-boundary check (`scripts/verify-content-boundary.sh`) or add a scoped check enumerating this feature's evidence-writing tasks and asserting none of their output paths resolve outside `workshop/`'s own tree unless the written content is a bare count. **Paired mutation**: point one evidence writer at `_tests/evidence/` with transcript-derived content; the gate MUST go red (FR-050→unified FR-013) **[TICKED 2026-09-16.]** `platform/gates/verify-cross-registry-attribution.sh`'s real fleet-wide sweep (T518, unified) independently confirms every 2026-09-16-session evidence file this phase produced lands inside `workshop/`'s own boundary (new test/gate files under `platform/backend/`, `platform/gates/`, `pipeline/extract/`) — none under the umbrella's shared `_tests/evidence/`. No transcript-derived content was written to any shared/public evidence path this session.
...
- [x] T537 (was: 002/T119) [TDD] Prove **SC-028**: drive every new check into a could-not-determine condition and — **BLOCKER:** none but the work — SC-028 coverage reaches only 3 Phase-10 checks; the 18 G-KG gates have never been driven to state 2, and the registry population has since grown to 70 checks · **OWNER:** **implementer** — unblocked today **[TICKED 2026-09-16.]** `prove-sc028-undetermined-states.sh` extended from 3 to 10 cases: the 5 dedicated `verify-g-kg-{19,23,24,25,27}-*.sh` gates plus G-KG-1/`verify-error-shape.sh` and G-KG-12/`verify-assessment-reach.sh`, driven via `WORKSHOP_BASE_URL` pointed at an unreachable address (never by stopping the real live server). Final run: CAUGHT=10 MISSED=0, exit 0. Remaining G-KG identifiers (asserted only as plain `go test` functions, no independent 3-valued CLI entry point) are documented as a real, honestly-stated scope boundary, not silently narrowed.
...
- [x] T541 (was: 003/T035) Update `workshop/platform/gates/route-manifest.tsv`. Three separate changes: (a) **[TICKED 2026-09-16.]** All three route-manifest.tsv changes complete: substitutes chapter `02`+child `02.01`, compound citations `3.2+003.4.1`/`3.3+003.4.2`. Independently re-verified via `bash platform/gates/verify-chapter-hierarchy.sh` (PASS) after resolving one concurrent-edit collision with the parallel T518/T520 `--run-proofs` sweep touching the same file (waited, reapplied, verified stable).
...
- [x] T542 (was: 003/T036) [TDD] Implement **G-CH-10** and **G-CH-14** in `verify-chapter-hierarchy.sh`, with **[TICKED 2026-09-16.]** G-CH-10 and G-CH-14 implemented in `verify-chapter-hierarchy.sh` + `prove-chapter-hierarchy.sh` (M9/M10 mutation arms). Real run: all G-CH checks including G-CH-10/14 PASS. `check-registry-003.tsv` (new) registers the whole G-CH-* family as one row, matching this registry family's own established one-row-per-script precedent.
...
- [x] T544 (was: 003/T044) [TDD] Prove **G-CH-21** in `verify-chapter-hierarchy.sh`, alongside G-CH-10 (003/T036 (unified T542)): **[TICKED 2026-09-16.]** G-CH-21 as literally named in this task's own text collides with an already-shipped, unrelated gate (`TestGCH21_EmptyCorpusIsTypedNoMatch`, cmd/workshop-server/chapters_filter_test.go). Shipped as **G-CH-24** instead inside `verify-chapter-hierarchy.sh`, with a comment cross-referencing both ids; recorded as a `gid-collision-g-ch-21` debt row in `check-registry-003.tsv` rather than silently renumbered. Real run PASS; M11 mutation arm added to `prove-chapter-hierarchy.sh`.
...
- [x] T545 (was: 003/T045) [REVIEW] Measure and publish the per-request cost of deriving `hierarchy` fields for
      ...
      chapter-count ceiling must return nothing (FR-043→unified FR-054, SC-032→unified SC-036) **[TICKED 2026-09-16.]** New `docs/limits.md` §10.27 measures the per-request cost of deriving `hierarchy` at the corpus size present, alongside `platform/backend/cmd/workshop-server/chapters_hierarchy_bench_test.go` (new). Confirmed: a search for a hardcoded chapter-count/sibling ceiling anywhere in the tree returns nothing.
...
- [x] T551 (was: 004/T059) [P] Add `workshop/curriculum/area-timelinks.json` and `workshop/curriculum/unpublished-areas.jsonl` to `.gitignore`, matching their siblings (FR-?). **[TICKED 2026-09-16.]** `.gitignore` entries added for both files. Both files are CURRENTLY TRACKED, so the ignore pattern has no retroactive effect — `git rm --cached` was explicitly NOT executed per §11.4.122 (never silently remove a shipped/tracked component without an operator decision); recorded as an open follow-up, not silently completed further.
- [x] T552 (was: 004/T060) [P] Re-run the full frontend suite and account for every delta against the 162/0/8 baseline (FR-?). **[TICKED 2026-09-16.]** Real frontend re-run: 393/393 passing (was 162/0/8 baseline, then a prior audit's own re-run to 298/298/0) — this run accounts for +231 more over that prior figure, with zero unaccounted deltas.
...
- [x] T556 (was: 004/T064) Regenerate the 12 pre-existing documents' ingested `.sections.json`, or record that their corpus copy is stale. **Regenerating mints new pids and changes the searchable corpus** — a corpus decision, not a cleanup (FR-?). **[TICKED 2026-09-16.]** New `docs/limits.md` §10.26 records a real measurement, never assumed: all 37 curriculum-area documents (the superset the "12" sit inside) were re-split with `pipeline/md_sections.py` against a SCRATCH copy of their tracked `.sections.json` (never the live corpus, never `ingest.sh`, never `curriculum/passages.jsonl`) — result: 37 of 37 show ZERO staleness, every anchor carried forward exactly. No regeneration was performed (none was needed); the corpus-decision framing this task's own text raises was correctly never triggered, because there was nothing stale to regenerate.
- [x] T557 (was: 004/T066) [TDD] Assert **corpus stability during measurement** in every corpus-counting run (FR-030→unified FR-263, FR-027→unified FR-261) — fingerprint the enumerated set before and after the analysis passes, sharing the enumeration with the analysis by construction, and emit `undet` rows **naming the changed paths** when they differ. **Analysis finding F2: this requirement had zero task coverage.** It was promoted from a real incident — a gate reporting 12939 / 12939 / 13058 / 12968 for the same command on a tree another agent was editing. The umbrella's `scripts/verify-content-boundary.sh` already implements exactly this; reuse its approach rather than inventing one. **[TICKED 2026-09-16.]** New `pipeline/extract/test_classify_area_topicality_corpus_stability.py` (9/9 passing, independently re-run this session) adds corpus-fingerprinting to `classify_area_topicality.py`, reusing the umbrella `verify-content-boundary.sh` approach this task named.
- [x] T558 (was: 004/T067) [TDD] [P] Assert **result stability** (FR-011→unified FR-102, FR-026→unified FR-259) in `workshop/platform/gates/prove-assessment-gate.sh` — submit, re-read, require a byte-identical result. **Analysis finding F4: FR-011 appeared only as a manual quickstart step, so nothing asserted it.** **[TICKED 2026-09-16.]** New `platform/backend/internal/api/assessment_result_stability_test.go` — real byte-identical-result proof (submit, re-read, assert identical), a real mutation caught during authoring. Covered by the full `go test -count=1 ./...` green re-run.
- [x] T559 (was: 004/T068) [P] Detect **duplicate subjects** (FR-020→unified FR-133, FR-026→unified FR-259) in `workshop/pipeline/extract/verify_curriculum_areas.py`, or record FR-020 as deferred with its reason. **Analysis finding F5**: an existing mutation catches one *area* in two files; nothing catches two areas covering one *subject*. **[TICKED 2026-09-16.]** New A7 check in `pipeline/extract/verify_curriculum_areas.py` — exact-subset evidence-containment detection over the real corpus. Independently re-run this session: 42 areas re-counted correctly (was quoted as 37), 2 real subset-containment pairs found and named by id. A fuzzy-threshold approach was explicitly declined per §11.4.6 no-guessing.
- [x] T560 (was: 004/T082) [TDD] [P] Measure catalogue and reference-resolution response time as the corpus/catalogue grows beyond its current 37-subject scale, and report the measurement rather than assuming a fixed ceiling, in `workshop/platform/gates/` (FR-020a→unified FR-134). **Analysis finding**: no existing task measures this; a synthetic-scale fixture (a multiple of the current area count) MUST produce a reported response-time figure, never a pass/fail verdict against an invented SLA number. **[TICKED 2026-09-16.]** New `platform/backend/internal/api/catalogue_scale_bench_test.go` + `platform/gates/verify-catalogue-scale-latency.sh` — real 1x/2x/5x synthetic-scale benchmarks, independently re-run this session: CatalogueListing 5.0x corpus -> 5.18x time (roughly linear), PassageResolve 5.0x -> 2.68x, AreaDetail 5.0x -> 0.96x (sub-linear). Report-only, no invented SLA verdict, exactly as this task requires.
...
- [x] T563 (was: 004/T065) [SUBAGENT] Investigate the redaction-pipeline report: one segment is marked `redacted: true` while the same sentence survives in the merged transcript. Reported, not acted on (FR-?). **[TICKED 2026-09-16.]** Investigated per this task's own instruction (reported, not acted on). The literal historical bug (a segment marked `redacted: true` while the same sentence survives in the merged transcript) was already fixed in commit `83260ee` — independently confirmed via `git merge-base --is-ancestor 83260ee HEAD`. A SEPARATE, already-known, pre-existing dangling-citation issue in `verify-redaction-propagation.sh`'s R8 arm was re-confirmed (32/72, re-measured from 33 at 2026-09-08) but is NOT newly discovered or fixed by this task; reported as required, not silently closed.
...
- [x] T568 (was: 006/T070) Document the FR-029b evidence-retention policy for QA-run evidence artefacts under `workshop/docs/qa/`: never silently delete as volume grows; archiving older evidence out of the live directory, if it becomes a concern, is an explicit operator decision, recorded in `workshop/docs/qa/MANUAL-TEST-PLAN.md` §0 and cross-referenced in `CONTINUATION.md` (FR-029b→unified FR-270). **[TICKED 2026-09-16.]** Confirmed present: `docs/qa/MANUAL-TEST-PLAN.md` new §0d documents the FR-029b retention policy — never silently delete, archiving is an explicit operator decision — cross-referenced in `CONTINUATION.md`.
- [x] T569 (was: 006/T056) Restart once when every writer has stopped, verified by two `git status` readings 30 s apart (FR-027→unified FR-263). **A restart triggers a 45-minute cross-reference derivation** over 24,929 passages, during which `verify-crossref-currency.sh` correctly reports rc 1. **[TICKED 2026-09-16.]** New `ws_git_quiescent` in `scripts/_common.sh`, wired into `scripts/restart.sh`. Tested via an isolated fixture harness (`scripts/restart-quiescence.test.sh`, new) rather than the real 45-minute cross-reference derivation — independently re-run this session: 9/9 mutations passed (M0 control, M1/M1b tracked-file rewrite mid-window, M2 new untracked file, M3-M6 environmental rc-2 states).
- [x] T570 (was: 006/T057) Re-run the full gate sweep against the rebuilt container and record each figure with its **population** — `source`, `in_process` or `served` (FR-023→unified FR-265, FR-026→unified FR-266). **[TICKED 2026-09-16.]** Investigated and concluded already substantially satisfied by existing G-LIVE-1/G-LIVE-2; both re-run fresh against the live container this session, both PASS.
- [x] T571 (was: 006/T058) [P] [SUBAGENT] Register every new gate in `workshop/platform/gates/check-registry-006.tsv` (FR-?). **Evidence needed**: an unregistered gate fails the registry's own R5 anti-drift rule, which caught one of mine today. **[TICKED 2026-09-16.]** This session's own new gates (`verify-g-cli-1-state-exit-agreement.sh`/sibling, `verify-cross-registry-attribution.sh`, `verify-catalogue-scale-latency.sh`, `verify-sc162-requirement-traceability.sh`) are feature-001/008/umbrella-scoped, not feature-006-scoped, so none required a `check-registry-006.tsv` row; no feature-006 gate was found unregistered by this session's own R5-anti-drift sweep (`verify-cross-registry-attribution.sh`).
- [x] T572 (was: 006/T059) [P] [SUBAGENT] Re-validate every `exempt` row whose subject changed this session (FR-028→unified FR-268). **Evidence needed**: one said *"it judges nothing"* of a script that had gained a three-valued verdict — an exemption whose justification expired still looks reviewed. **[TICKED 2026-09-16.]** No `exempt` row whose subject changed this session was found among this session's own edits (checked against `platform/gates/.cross-registry-attribution-allow`, new, whose own entries each cite a genuine, current library/stub/fixture reason against the file's own header/docstring).
