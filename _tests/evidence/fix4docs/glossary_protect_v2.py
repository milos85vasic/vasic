...
def protect(glossary_path, map_path):
    ...
    for term in terms:
        ...
        num2term[str(n)] = term
        matched.append(term)
    # --- Coalesce adjacent-sentinel RUNS -------------------------------------
    # When >=2 glossary sentinels sit consecutively separated only by a short
    # list delimiter (", " / " / " / " & " ...), some engines (notably Kazakh)
    # truncate the run, silently dropping the TRAILING sentinels (the #45 gap).
    # Merge each such run into ONE atomic sentinel whose restore value is the
    # original terms rejoined with their ORIGINAL separators. The restored text
    # is byte-identical to the per-term result, so nothing that already passes
    # can regress; the engine can no longer partially drop a list.
    RUN_RE = re.compile(
        r"Z9TERM\d{1,4}Z9(?:[ \t]*[,/;&|]+[ \t]*Z9TERM\d{1,4}Z9)+")
    coalesced_consumed = []

    def _coalesce(m):
        nonlocal n
        seg = m.group(0)
        parts = re.split(r"(Z9TERM(\d{1,4})Z9)", seg)
        # parts alternates: sep, sentinel, num, sep, sentinel, num, ...
        pieces = []
        nums = []
        i = 0
        while i < len(parts):
            if i + 2 < len(parts) and parts[i + 1] and SENTINEL_RE.fullmatch(parts[i + 1] or ""):
                pieces.append(parts[i])            # leading separator ("" first)
                num = str(int(parts[i + 2]))
                if num not in num2term:
                    return seg                     # safety: bail, leave as-is
                pieces.append(num2term[num])       # the term text
                nums.append(num)
                i += 3
            else:
                pieces.append(parts[i]); i += 1
        if len(nums) < 2:
            return seg
        joined = "".join(pieces)
        n += 1
        newsent = "Z9TERM%03dZ9" % n
        num2term[str(n)] = joined
        coalesced_consumed.extend(nums)
        return newsent

    text = RUN_RE.sub(_coalesce, text)

    with open(map_path, "w", encoding="utf-8") as f:
        json.dump({"num2term": num2term, "matched": matched,
                   "coalesced_consumed": coalesced_consumed},
                  f, ensure_ascii=False, indent=2)
    sys.stdout.write(text)
    sys.stderr.write("[protect] %d glossary terms protected: %s\n" % (len(matched), ", ".join(matched)))
    if coalesced_consumed:
        sys.stderr.write("[protect] coalesced %d adjacent sentinels into list-runs\n" % len(coalesced_consumed))
    return 0
...
def restore(map_path):
    with open(map_path, encoding="utf-8") as f:
        m = json.load(f)
    num2term = m["num2term"]
    consumed = set(m.get("coalesced_consumed", []))  # merged into a list-run sentinel
    text = sys.stdin.read()
    unknown = []
    seen = set()
    ...
    # Constituents folded into a coalesced list-run no longer appear as their own
    # sentinel; the run sentinel carries them, so exempt them from drop-detection.
    dropped = [n for n in num2term if n not in seen and n not in consumed]
    errs = []
    if unknown:
