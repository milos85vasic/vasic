...
- **[Catalogizer](../products/Catalogizer.md)** — multi-protocol (SMB/FTP/NFS/WebDAV/local), encrypted, self-hostable media collection management; Go/Gin API + React UI; resilient offline-tolerant monitoring; built on 21 `digital.vasic.*` submodules.
- **[Courses-Creator](../products/Courses-Creator.md)** — markdown-to-video course pipeline; multi-LLM enrichment, TTS (Bark/SpeechT5), desktop/mobile/web players; graceful no-API-key mode.
- **[VisionEngine](../products/VisionEngine.md)** — decoupled Go toolkit fusing classic CV with multi-provider LLM vision; navigation graphs with BFS + DOT/JSON/Mermaid export; OpenCV build-tag-gated.
- **[DocProcessor](../products/DocProcessor.md)** — documentation-to-feature-map with verification-coverage tracking; LLM or heuristic/offline extraction; Apache-2.0.
- **[Docs Chain](../products/docs_chain.md)** — content-hashed, bidirectional, atomic doc/DB sync (Salsa-style incremental recompute over a DAG). *Phases 1–5 GREEN; Phases 6–7 PLANNED.*
- **[Herald](../products/Herald.md)** — reliable multi-channel notifications with natural-language, three-tier intent resolution (command → LLM → clarify); first Docs Chain consumer.
- **[task_bridge](../products/task_bridge.md)** — decoupled, bidirectional task/board sync (SQLite SSoT ↔ docs ↔ ClickUp). *P1 scaffold — sync logic not yet implemented.*
- **[Vasic Digital Reusable Module Suite](../products/Vasic-Digital-Reusable-Module-Suite.md)** — the `digital.vasic.*` "standard library": infrastructure primitives, AI building blocks, and defensive-LLM guardrails, plus a Kotlin Multiplatform mirror. *Several org repos are SCAFFOLD/WIP — flagged, not shipped.*
...
- **[Mail Server Factory](../products/Mail-Server-Factory.md)** — declarative JSON → fully-provisioned, Dockerized mail server across 12 connection types and 25 Linux distributions; enterprise security; reports 439 passing tests and a clean SonarQube gate. Flagship of the Server-Factory org.
- **[Server Factory Core Framework](../products/Server-Factory-Core-Framework.md)** — the shared Kotlin engine every factory builds on.
- **[Qemu-Utils](../products/Qemu-Utils.md)** — QEMU VM images managed like artifacts: download/cache/run, compress/publish, bridge/TAP networking, ISO installs; Linux + macOS.
- **[Parallels-Utils](../products/Parallels-Utils.md)** — Parallels (macOS) VM image compression, publishing, and retrieval via simple settings files.
- **[Server Factory — Additional Components](../products/Server-Factory-Additional-Components.md)** — service factories (Web/SonarQube/Caching-Proxy), Definitions packs, and Utils. *Service factories are placeholder-documented — UNVERIFIED / early-stage.*
