...
> ⚠️ **SUPERSEDED (2026-08-10).** This 2026-06-17 spec predates the OpenDesign
> mandate (§11.4.162) and the Go-generator rebuild. Its "Current State" section
> below describes the **obsolete** pre-rebuild stack (hand-authored
> `css/style.css`, `js/translations.js`, per-page HTML) that **no longer exists** —
> both sites are now generated by `_tools/gen` from `_content/**` + OpenDesign
> tokens (`design-system/brand-*`). The bespoke CSS-custom-property token system
> this doc prescribes is now **forbidden** by §11.4.162. This document is retained
> for historical context only; the governing plan is
> [`OPENDESIGN-ADOPTION-PLAN.md`](./OPENDESIGN-ADOPTION-PLAN.md), which supersedes
> it. Do not treat anything below as the current architecture.
