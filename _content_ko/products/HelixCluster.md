...
- **크로스 플랫폼 동등성(리눅스 전용 모킹 금지).** 빌드 태그를 통해 공유 인터페이스를 실제 OS별 기능으로 분할하여 해결 — 리눅스 cgroup/`/proc`/커널 WireGuard, macOS `sysctl`/`vm_stat`/IOKit/`wireguard-go` — 그리고 독립적인 OS 오라클과 교차 검증을 통해 각 플랫폼이 리눅스 가정이 아닌 진정한 네이티브 상태를 보고하도록 합니다.
