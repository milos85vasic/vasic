...
- **التكافؤ عبر المنصات (عدم الاقتصار على محاكيات Linux فقط).** تم حل هذه المشكلة بواجهة مشتركة مقسمة عبر علامات البناء إلى مرافق أصلية لكل نظام تشغيل — مثل مجموعات التحكم في Linux (`cgroup`) و`/proc` وواجهة نواة WireGuard، و`sysctl` و`vm_stat` وIOKit و`wireguard-go` في نظام macOS — ثم التحقق المتبادل مع مرجع نظام تشغيل مستقل، بحيث يقدم كل نظام حالة أصلية حقيقية بدلاً من افتراضات Linux.
